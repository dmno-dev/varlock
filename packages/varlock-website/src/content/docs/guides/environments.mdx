...
Process overrides are applied when varlock _resolves_ your config: during `varlock run`, `varlock load`, or a framework integration running at dev/build time. Some deployments boot from env values that were already resolved and baked in earlier, for example a framework integration's build output on a serverless platform, or a standalone server image booted without the varlock CLI. In those deployments the process env at boot is not an override source: resolution already happened, so those values are ignored, and varlock logs a warning naming the keys rather than passing over the mismatch silently. To have boot-time env vars respected as overrides, boot through [`varlock run`](/reference/cli/load-and-run/#run) (see the [Docker guide](/integrations/docker/)).
:::

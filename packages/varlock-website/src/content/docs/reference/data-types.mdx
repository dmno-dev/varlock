...
```env-spec
INFERRED_STRING_QUOTED="hello"
INFERRED_STRING_UNQUOTED=hello
...
CONCAT_INFERS_STRING=`concat-${APP_NAME}-will-be-string`
FN_INFER_BOOLEAN=eq($APP_ENV, $TARGET_ENV)
...
- `min` / `max`: bounds in any duration format (for example `min="1s"`, `max="1d"`)
...
**Array options:**
- `separator` (string, default `","`): splits plain-string input (for example a real environment variable override) and joins the value back into a string for `process.env`
...
In application code, `ENV.ALLOWED_EMAILS` is a real typed array (for example `string[]` or `number[]`, via [type generation](/guides/code-generation/)). In `process.env` the value is the flat string form (separator-joined, or JSON when `format=json`).
...
An untyped item whose value is an array literal (for example `ITEM=[a, b]`) is inferred as an array automatically.
</div>
...
**Object options:**
- `keyType`: a type used to validate every key, for example `keyType=enum(us, eu)` or `keyType=string(matches="[a-z]+")`
...
An untyped item whose value is an object literal (for example `ITEM={k=v}`) is inferred as a record automatically. The existing `simple-object` type behaves the same as a bare `record`.
</div>
</div>

...
Varlock scans outgoing HTTP responses at runtime to detect if any sensitive values are being accidentally sent to clients. If a leak is detected, varlock throws an error with a detailed diagnostic message including the config item key and where the leak was detected. The rejected body is never delivered. When the leak is caught on `ServerResponse.end` (for example Next.js Pages Router `res.json()`, which sends the whole body in one call) varlock also finishes the response before rethrowing, so the client is not left waiting on a body that will never arrive: it gets a plaintext 500 if the headers have not gone out yet, and otherwise the connection is closed. A leak caught part-way through a streamed response always closes the connection, since nothing already sent can be taken back.

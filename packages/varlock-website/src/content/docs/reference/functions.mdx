...
- `tokenUrl=S`: the provider's token endpoint (required unless a provider instance supplies it). Must be https (plain http is allowed for localhost).
...
- `clientId=R`: the OAuth client id (required unless a provider instance supplies it)

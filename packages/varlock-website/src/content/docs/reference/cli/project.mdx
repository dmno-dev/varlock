...
- Files copied from outside the git repo are flagged with a warning. They are still copied, but they are not part of the project and will not be present for anyone else running the same build.

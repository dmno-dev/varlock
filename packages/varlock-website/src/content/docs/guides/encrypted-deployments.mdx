...
Nothing to set up. When `@encryptInjectedEnv` is enabled, the Vite plugin mints a temporary key the moment it is imported by your Vite config, before any plugin hooks run. Frameworks that run the SSR dev runtime in a separate worker or process (Nitro and TanStack Start, vitest) spawn it later with a copy of the environment, so they inherit the key and the blob is encrypted in dev the same way it is in production. The key lives only in memory for that dev session and is never used for builds. If you export `_VARLOCK_ENV_KEY` yourself before starting the dev server, that key is used instead.

...
Decryption uses `node:crypto` where available; edge runtimes without it (for example Vercel Edge middleware) automatically fall back to asynchronous Web Crypto, with request handling gated until the env is ready.
...
1. **Generate and set the key**
    ...
    <Tabs>
      ...
      You can use the same key for both `preview` and `production`, but we recommend separate keys because it isolates environments and reduces blast radius if one key is exposed.
      </TabItem>

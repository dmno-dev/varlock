...
There are a handful of ways to combine varlock with sandboxed agents, ordered here by how far real secrets stay from the agent. The provider guides below each bless one path (the broker instance, for most providers) rather than documenting all of them.
...
4. **Proxy on infrastructure you run.** The same broker shape, but on a VM or container platform you already operate, reachable at a URL that carries WebSockets. Useful when one broker needs to serve fleets across providers, or outlive any one provider's sandbox lifecycle.

...
Manage the encrypted value cache used by [`cache()`](/reference/functions/#cache) and plugin authors. When run in a TTY, opens an interactive browser; otherwise prints a status summary.

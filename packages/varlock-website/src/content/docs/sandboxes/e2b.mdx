...
- No human is attached to the broker, so its policy must run unattended: allow rules, `block` rules, `@proxy=omit`, and strict egress. To change policy, write the edited schema with `files.write` and run `varlock proxy reload` via `commands.run`: rule changes apply to agent traffic immediately, and a newly added key shows up for newly started `proxy run` commands.

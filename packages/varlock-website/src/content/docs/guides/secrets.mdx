...
Marking an item `@sensitive` is a claim that varlock will keep the value out of your output. Redaction makes good on that by replacing the value wherever it appears, which only works for a string long enough not to occur in ordinary text. Varlock rejects values that can't be protected that way, rather than letting the decorator promise something it isn't delivering.

| The value | What happens |
| --- | --- |
| a number or boolean | error |
| an array or object with non-string elements | error |
| under 3 characters | error |
| under 12 characters | warning |
| the [`@currentEnv`](/reference/root-decorators/#currentenv) item | error |
| a secret embedded in a non-sensitive item's value | error, on the non-sensitive item |

These apply however the item became sensitive. `@defaultSensitive=true` is the default, so in a hand-written schema every port, feature flag and env name is sensitive until you say otherwise. If that produces a lot of errors, the fix is usually `@defaultSensitive=false` at the top of the file with an explicit `@sensitive` on each real secret, which is also what `varlock init` generates. Varlock deliberately errors here rather than quietly treating these items as public: the schema should say what is sensitive, and a reader (or an agent editing it) should not need to know varlock's rules to tell. Builtin `VARLOCK_*` variables are exempt.

**Numbers and booleans.** Redaction only replaces strings, so neither is ever masked. A boolean holds one bit, so there's no secret in it anyway. For a numeric secret, make it a string with `@type=string` or by quoting the value (`PIN="007123"`), which also preserves the leading zeros and the precision past 2^53 that a number drops. Otherwise mark it `@sensitive=false`.

**Composite values** are checked per element, since each element is registered for redaction on its own. A sensitive array or object must be made of strings, so `@type=array(number)` is rejected the same way a bare number is, and a long array with a one-character element is still a one-character match.

**Short values.** Redaction has no token boundary, so a sensitive `acmeco` rewrites every `acmeco` in your logs and proxied responses, including text that was never a secret. Set `@sensitive=false` when it isn't really a secret, like an org slug. Some secrets are short by nature, like a one-time code or a PIN; `@sensitive={allowShortValue=true}` records that you read the collision risk and accepted it. Under 3 characters the collision is a certainty, so that is an error and the acknowledgement doesn't apply.
...
**A secret inside a non-sensitive value** is reported on the non-sensitive item, since marking it sensitive is usually the fix. The public item carries the secret into logs, generated types and client bundles, where none of the sensitive item's protections apply, and redacting the secret rewrites the public value everywhere it legitimately appears. This is checked against what redaction actually registers, so it fires for a short acknowledged secret too (the collision is confirmed, not hypothetical) but not for a sensitive number, which is never in the map.
...
- [Azure Key Vault & App Configuration](/plugins/azure-key-vault/)

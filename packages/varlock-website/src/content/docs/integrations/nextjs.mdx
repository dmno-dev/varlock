...
Note that in the browser, reading a dynamic+public key *before* `loadPublicDynamicEnv()` completes returns `undefined` (unlike Vite-based frameworks, Next client bundles do not currently carry the declared dynamic key list, so varlock cannot raise its usual "not hydrated yet" error there). Gate rendering on the load completing, or handle the `undefined` initial state.

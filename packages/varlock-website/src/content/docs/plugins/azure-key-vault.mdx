...
Point the instance at your store with `appConfigEndpoint`. For Azure Government or Azure China, also set `cloud=usgov` or `cloud=china` so tokens are minted for the right endpoints. Authentication uses the same chain as Key Vault, and the identity needs the **App Configuration Data Reader** role on the store (see [Azure Setup](#app-configuration-data-reader-role) below).

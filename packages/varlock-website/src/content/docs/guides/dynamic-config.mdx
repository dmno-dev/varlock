...
There is one level past dynamic. `@dynamic=boot` says the value is bound at process start on each instance (a platform-assigned `PORT`, pod identity), so even a deploy-time pin like [`varlock freeze`](/guides/deploy-time-config/#keys-supplied-at-boot) leaves it to the runtime. It is a subset of dynamic and only changes behavior under freeze, so most schemas never need it.
...
Most recipes are the same shape: a small server route returning `getPublicDynamicEnv()`, and a one-time `loadPublicDynamicEnv()` call before browser code reads `ENV.KEY`. Frameworks that already ship server state to the browser (TanStack Start) can skip the endpoint and hydrate the values with `setPublicDynamicEnv()` instead.
...
- **Routes rendered per request (SSR)**: pass the values through the router. No endpoint, no fetch, no hydration mismatch.
- **Prerendered routes, cached HTML, or consumers outside the app** (a native client, another origin): use a server route plus `loadPublicDynamicEnv()`, like the other tabs. Anything not rendered per request freezes whatever the router saw at that moment.

TanStack Router calls `dehydrate()` on the server during SSR and awaits `hydrate()` in the browser before the first render, so `ENV.KEY` reads work in components from the initial render:

```ts title="src/router.tsx"
import { createRouter } from '@tanstack/react-router';
import { getPublicDynamicEnv, setPublicDynamicEnv } from 'varlock/env';
...
export function getRouter() {
  return createRouter({
    routeTree,
    dehydrate: () => ({
      publicDynamicEnv: getPublicDynamicEnv(),
    }),
    hydrate: (dehydrated) => {
      setPublicDynamicEnv(dehydrated.publicDynamicEnv);
    },
  });
}
```
...
function IndexPage() {
  return <p>{ENV.RUNTIME_FLAG}</p>;
}
```

If you keep these router options and also have routes that are prerendered or served from cached HTML, those pages ship a stale snapshot and `hydrate()` fills the store with it, which makes a plain `loadPublicDynamicEnv()` skip its fetch. Pass `force: true` on those routes and keep `ENV` reads out of render:

```tsx title="src/routes/prerendered.tsx"
import { useEffect, useState } from 'react';
import { loadPublicDynamicEnv } from 'varlock/env';

function PrerenderedPage() {
  const [runtimeFlag, setRuntimeFlag] = useState<string>();
  useEffect(() => {
    loadPublicDynamicEnv({ endpoint: '/api/public-env', force: true })
      .then((values) => setRuntimeFlag(values.RUNTIME_FLAG));
  }, []);
  return <p>{runtimeFlag ?? 'loading...'}</p>;
}
```

`/api/public-env` is a server route returning `getPublicDynamicEnv()`, as in the other recipes.

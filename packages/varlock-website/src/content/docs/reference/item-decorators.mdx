...
- Under 4 characters: an error, since there is no output in which redacting a value that short is harmless. `@sensitive={allowShortValue=true}` still opts in.
- A boolean value: never sensitive. A boolean holds one bit, so there is no secret in it to protect, while redacting it would rewrite every `true`/`false` in logs and proxied response bodies. Booleans are treated as `@sensitive=false` automatically; an explicit `@sensitive` on one is ignored, with a warning. (Note this also makes them [`@static`](#static) by default, so a boolean read at runtime needs an explicit `@dynamic`.)
...
Only booleans are demoted rather than rejected. For every other type the value could hold a real secret, and quietly making it public would also make it `@static` and eligible to be inlined into a build, so varlock stops (or warns) instead of guessing.

...
- `refresh`: Hot-reload a daemon started with `--allow-reload` after an intentional schema edit; re-resolves and swaps the live policy without restarting. (Off by default; otherwise restart the proxy to apply schema changes.)
- `stop`: Stop a session.
...
- `--allow-reload`: For `start`/`run`, enable live policy hot-reload via `proxy refresh` (off by default; the reload channel is unauthenticated on a shared uid, so only enable behind a sandbox).

...
and also may affect other dynamic behaviour in your schema, such as the [`forEnv()` function](/reference/functions/#forenv).
...
May also be used with [`forEnv()`](/reference/functions/#forenv) to disable a file based on the current env flag.
...
Imports config items from another `.env` file or a directory containing `.env*` files. Useful for sharing values across services, splitting large schemas, or pulling selected keys from another source.
...
See the [@import guide](/guides/import) for detailed examples and advanced usage.

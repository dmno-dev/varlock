...
- `processEnv`: how `process.env` is typed — `strict`, `loose`, or `none` (defaults to `strict`). Note that `@types/node` declares a base string index signature that can't be removed, so extra keys remain allowed on `process.env` regardless.

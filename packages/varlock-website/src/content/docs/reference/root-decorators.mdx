...
- `auto` (default): uses `memory` in CI, or when local encryption is using file-based fallback, uses `disk` otherwise

You can of course use functions to dynamically set cache mode based on environment or other config values.

:::note
`--skip-cache` on `varlock load` / `varlock run` disables cache reads and writes for that invocation, regardless of `@cache`.
:::

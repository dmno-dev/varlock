...
Where it does matter is when build and deploy are decoupled: for example, building client assets once and deploying that same build to several environments, where deploy-time resolution gives each environment different values. An inlined public value is frozen from the build; a public+dynamic value can instead be served by the worker with its deploy-time value.
For the shared model and tradeoffs, see the [Static vs Dynamic Config guide](/guides/dynamic-config/).

There is no automatically injected endpoint on Cloudflare. For that decoupled setup, add a route in your worker that serves `getPublicDynamicEnv()`:
...
The endpoint serves whatever the worker was deployed with, not per-request-fresh values.

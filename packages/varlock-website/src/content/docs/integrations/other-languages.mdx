...
fn main() {
    // `env::ENV` is loaded once and cached; use `env::load()` if you'd rather handle errors
    let port = env::ENV.db_port;   // Option<i64>
    let debug = env::ENV.debug;    // Option<bool>
    println!("{:?} {:?}", port, debug);
}
```
...
func main() {
    // env.Get() is loaded once and cached; use env.Load() if you'd rather handle errors
    e := env.Get()
    _ = e.DbPort   // *int64
    _ = e.Debug    // *bool
}
...
// Env::instance() is loaded once and cached; use Env::load() for a fresh parse
$env = Env::instance();
$port = $env->DB_PORT;    // ?int
$debug = $env->DEBUG;     // ?bool

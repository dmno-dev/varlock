...
- `cacheTtl` (optional): when set, resolved values from `op()` and `opLoadEnvironment()` are cached locally for the specified duration. Accepts the same format as the [`cache()` function](/reference/functions/#cache) — e.g., `"5m"`, `"1h"`, `"1d"`, or `0` for forever.

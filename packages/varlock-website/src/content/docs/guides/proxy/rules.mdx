...
Either side can also interpolate, e.g. `username="acct-${TENANT}"` or `password="${TOKEN}-suffix"`. The literal text and the references are kept apart in the rule and composed when the proxy signs, so an interpolated credential's value stays out of rule data exactly like a bare reference. Every item referenced this way is a consumed credential: managed, never substituted, and leak-guarded.

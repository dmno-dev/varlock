...
- `strict`: add `@proxyConfig={egress="strict"}` to the header so only requests matching a `@proxy` rule are allowed; everything else is blocked. This is the recommended posture once your rules are dialed in, since it prevents the agent from reaching arbitrary hosts.
...
:::caution[Approval rules fail closed in one-shot mode]
A self-contained `proxy run` has no terminal to prompt in (the child owns its stdio), so requests matching an approval-required rule are denied with a 403. For interactive approvals, run `proxy start` in one terminal and let `proxy run` attach from another.
:::
...
It's **opt-in** by design (a jail that's too tight silently breaks an agent, so we don't force it on). macOS only today; on Linux/Windows use the bring-your-own path below. It applies to a self-owned one-shot run — when a `proxy start` daemon is running, `--sandbox` on the attaching `proxy run` still sandboxes the child it spawns.

### Bring-your-own: containers & VMs

For stronger isolation (or on Linux/Windows), run the agent inside a container or VM (Docker, a devcontainer, Apple `container`, microsandbox) with all egress routed through the proxy on a host-reachable address. This is the VM-grade tier and covers the filesystem and process table too, not just network + credentials. See the sandbox guide for per-runtime recipes.

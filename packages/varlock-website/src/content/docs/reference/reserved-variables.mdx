...
Fallback for the [`--filter`](/reference/cli-commands/#filtering-items) flag on [`varlock load`](/reference/cli-commands/#load) / [`varlock run`](/reference/cli-commands/#run). An explicit `--filter` flag takes precedence over this env var. Useful when you can't easily pass a CLI flag, for example a wrapper script, CI config, or a build-time integration that invokes varlock's loading APIs directly.
...
The serialized env graph (resolved config values plus metadata) injected by [`varlock run`](/reference/cli-commands/#run) and build-time integrations, so the runtime can load your config without re-invoking the CLI. [`@internal`](/reference/item-decorators/#internal) items are excluded from this blob by default everywhere it's built, including framework integrations.

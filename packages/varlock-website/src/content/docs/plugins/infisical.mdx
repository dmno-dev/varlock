...
If you are using the Infisical EU Cloud, you must set `@initFisical(siteUrl=https://eu.infisical.com)`

(See [Universal Auth Documentation](https://infisical.com/docs/documentation/platform/identities/universal-auth))
:::
...
You can also set `allowMissing` on `@initInfisical()` to apply it to all `infisical()` calls for that instance. It can be a dynamic value, for example `forEnv(development)` to only allow missing secrets in dev:
...
- `environment` (required): Environment name (for example `dev`, `staging`, `production`)
- `clientId` (required): Universal Auth Client ID. Reference a config item of type `infisicalClientId`.
- `clientSecret` (required): Universal Auth Client Secret. Reference a config item of type `infisicalClientSecret`.
...
- `allowMissing` (optional): if `true`, missing secrets return `undefined` instead of throwing. Can be a dynamic value (for example `allowMissing=forEnv(development)`). Other errors (such as auth failures and permission errors) still throw.
...
**Key/value args:**
- `allowMissing` (optional): if `true`, returns `undefined` instead of throwing when the secret is not found. Can be a dynamic value (for example `allowMissing=forEnv(development)`). Useful with `fallback()` to provide a default value. Other errors still throw.

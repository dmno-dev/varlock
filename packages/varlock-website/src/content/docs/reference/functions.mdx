...
- Starts and ends with `/` (with optional flags like `d`, `i`, `g`, `m`, `s`, `u`, `v`, `y` after the closing `/`)
- Is **not** wrapped in quotes

This detection happens after parsing, so the pattern is still an ordinary unquoted value first. A `,`, a space, or a `)` ends a value, so a pattern containing any of them cannot be written this way and must use [`regex()`](#regex) instead. A `{n,m}` quantifier is the common case:

```env-spec
# the comma in {7,40} would end the value, so this needs regex()
...
:::note[`regex()` function]
`regex("pattern")` takes the pattern itself as a quoted string, with optional flags as a second argument. Do not wrap it in slashes: `regex("/^abc$/")` would match the slashes too, so it is rejected with an error. Prefer the `/pattern/` form when it fits, since it is more concise. Reach for `regex()` when the pattern contains a `,`, a space, or a `)`, or when it has enough `/` in it that escaping each one hurts readability.

```env-spec
# quoted pattern, so a comma inside it is fine
# @type=string(matches=regex("^[0-9a-f]{7,40}$"))
APP_COMMIT_SHA=

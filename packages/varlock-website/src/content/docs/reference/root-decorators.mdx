...
<div>
### `@proxyConfig`
**Value type:** `{egress?: "permissive" | "strict"}`

Configures [credential proxy](/guides/proxy/) egress. The proxy is driven by [`@proxy`](/reference/item-decorators/#proxy) decorators on your items, so this header decorator is **optional**: it exists only to change the egress mode from the default. It is a single-use, object-value decorator (`@proxyConfig={...}`), not a function call.

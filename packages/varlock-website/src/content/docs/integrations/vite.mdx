...
Marking a value `@dynamic` needs no extra code: server-side reads work as usual, with values resolved when the server boots (not per request). The endpoint + browser hydration machinery is only needed when **browser code** reads a public+dynamic value (it must be fetched from the backend), which is a niche situation. See [When do you need the client-side machinery?](/guides/dynamic-config/#when-do-you-need-the-client-side-machinery).

If you do need browser access, see the [Vite recipe in the guide](/guides/dynamic-config/#framework-recipes).

...
- The Windows helper is only launched when varlock actually needs it: decrypting or encrypting a value, or writing to the encrypted disk cache. Loading a schema with no encrypted values never invokes it, so it adds no startup cost on WSL

...
- `--url <wss>` + `--token <token>`: For `run`, target a proxy on another machine (a broker started with `--tunnel`) over the tunnel, instead of a local session. The token is the broker's data-plane token (also readable from `VARLOCK_PROXY_TOKEN`). The local-proxy flags (`--sandbox`, `--port`, `--cert-dir`, `--persist-ca`, `--tunnel`) don't apply with `--url`.

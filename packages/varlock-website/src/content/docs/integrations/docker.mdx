...
| **Deploy-time freeze** | Apps with no build step that would inline values (Elysia, Hono, Fastify), where you want config pinned to the release | Sometimes. Only in a **builder** stage or CI |
...
For serverless SSR, prefer [encrypted deployments](/guides/encrypted-deployments/) over baking resolved values into image layers.
  ...
  <TabItem label="Deploy-time freeze">

Use when your app has no build step to inline values, but you still want config resolved once per release rather than on every container boot. [`varlock freeze`](/guides/deploy-time-config/) resolves everything at deploy time and encrypts it; the runtime image needs no varlock CLI, no `.env` files, and no resolver credentials, so this works in distroless.
...
# .env files and resolver credentials exist only in this stage
...
Set `_VARLOCK_ENV_KEY` in the runtime environment so the file can be decrypted, and `_VARLOCK_USE_FROZEN_ENV=1` so a missing file is an error rather than a silent fall back to normal resolution.

If you cannot copy a file into the image (a Compose file that pulls a tag it does not rebuild, say), `varlock freeze --out -` writes the same payload to stdout and you carry it in one variable instead:
...
That is a weaker seal, since the payload lives in platform config rather than inside the release. See [carrying the payload in an env var](/guides/deploy-time-config/#carrying-the-payload-in-an-env-var-instead) for the full comparison.

Values are pinned either way: rotating a secret takes effect on your next deploy, not on the next restart.
...
With `varlock run` as the entrypoint, varlock is PID 1. It forwards `SIGTERM`, `SIGINT`, `SIGHUP`, and `SIGQUIT` to your app's process group, waits for the app to finish its own shutdown handlers, and exits with the app's status. `docker stop` and Kubernetes pod termination reach your app the same way they would if it were PID 1 itself. See [Signals and exit codes](/reference/cli/load-and-run/#run) for details.

What varlock does not do is reap orphaned processes. If your app spawns subprocesses that can outlive their parent (a shell that backgrounds a job, a worker pool that forks), the orphans are reparented to PID 1 and remain as zombies once they exit. If that applies to you, put a minimal init above varlock. Signal forwarding works the same either way:

```dockerfile title="Dockerfile"
RUN apk add --no-cache tini
ENTRYPOINT ["tini", "--", "varlock", "run", "--"]
CMD ["node", "dist/server.js"]
```

`docker run --init` does the same thing without changing the image, but it is a Docker flag, so it does not apply under Kubernetes or other orchestrators.
...
[`varlock flatten`](/reference/cli/project/#flatten) collapses the whole import graph into one self-contained directory (default `.env-flat/`) and rewrites the `@import` paths, so the package's env files no longer reach outside the package:
...
A few behaviors worth knowing (see the [command reference](/reference/cli/project/#flatten) for the full list):
...
In **Node-based images** where varlock is an npm dependency, install plugins in `package.json` and reference them without a version (or with a range for validation only). See the [Plugins guide](/guides/plugins/#installation) for details.
...
See [Monorepos and partial build context](#monorepos-and-partial-build-context). The error usually lists a path outside your Docker build context. Run [`varlock flatten`](/reference/cli/project/#flatten) while the full monorepo is available, add explicit `COPY` instructions, or widen the context to the monorepo root.

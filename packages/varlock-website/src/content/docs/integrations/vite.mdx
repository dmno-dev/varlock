...
With `resolved-env`, the baked values are authoritative at runtime. If the runtime environment provides a different value for a schema-declared key, varlock logs a loud warning naming the keys and boots on the baked values, since runtime values cannot be validated or applied to a baked snapshot. A fresh `__VARLOCK_ENV` blob injected at boot (e.g. via [`varlock run`](/reference/cli/load-and-run/#run)) takes precedence over the baked payload.

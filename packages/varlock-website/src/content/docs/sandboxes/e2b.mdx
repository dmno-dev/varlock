...
- No human is attached to the broker, so its policy must run unattended: allow rules, `block` rules, `@proxy=omit`, and strict egress. To change policy, write the edited schema with `files.write`, then `commands.run` a reload with the same shape as the start command (`proxy reload` validates the schema in its own process, so it needs the project dir and the bootstrap `envs`): `` cd /home/user/proj && ${VARLOCK} proxy reload ``. Rule changes apply to agent traffic immediately; a newly added key shows up for newly started `proxy run` commands.

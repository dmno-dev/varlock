...
OPENAI_API_KEY=yourPreferredPlugin() # from a plugin or built-in encryption — never a plaintext secret
```
...
Secrets never enter any container: the host proxy resolves them (keeping your encryption key, keychain/enclave, warm `op` session, and interactive approvals on the host) and injects the real value onto the wire; the container only ever holds placeholders. Both networks and the forwarder are torn down when the command exits.

...
When deploying to serverless platforms, varlock injects the resolved env into your server-side build output as plaintext JSON (when using `ssrInjectMode: 'resolved-env'`). This is generally safe since it only appears in server code, but you can encrypt it for extra protection (e.g., against sourcemap leaks). See the [encrypted deployments guide](/guides/encrypted-deployments/) for setup instructions.
:::

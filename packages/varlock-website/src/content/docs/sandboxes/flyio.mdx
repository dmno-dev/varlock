...
The broker holds real secrets inside Fly's cloud, so their infrastructure is inside your trust boundary, same as it would be for secrets passed to any sprite. What changes is the blast radius on your side: agents never hold secrets, and rotation, policy, and audit live in one place instead of N sprites. The tunnel token is the credential to *use* the broker, not to read its secrets; rotate it by recreating the service with a new one. To change policy, upload the edited schema the same way and restart the service (`sprite api /v1/sprites/varlock-broker/services/varlock-proxy/restart -- -X POST`): a restart re-reads the schema, and `--persist-ca` plus the pinned token mean running agents just reconnect. Note that re-`PUT`ting a service definition does not restart a running service, so the restart call is needed even if you also change the service env.

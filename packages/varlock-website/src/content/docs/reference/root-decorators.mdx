...
- `infer`: Items with a static or function value are required; items with an empty string or no value are optional. Can be overridden per item.
...
PUBLIC_FOO=bar   # not sensitive
SECRET_BAR=baz   # sensitive

# @sensitive=false
FOO=bar          # not sensitive (explicit)

# @sensitive=true
PUBLIC_BAR=baz   # sensitive (explicit)
```
</div>

<div>
### `@disable`
Skips loading config items from a file or data source. If present (or set to `true`), the file is ignored and no items are loaded from it. Useful for conditionally disabling a schema or env file.

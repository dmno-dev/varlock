...
Your `.env.schema` and `.env` files are resolved from the directory you run the command in (or wherever that directory's `package.json` sets `varlock.loadPath`), not from parent directories. Wrangler's `--cwd` moves that too. `--config` does not.

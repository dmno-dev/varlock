...
<div>
### `@currentEnv`
**Value type:** [`ref()`](/reference/functions/#ref) (written as `$ITEM_NAME` or `ref(ITEM_NAME)`)

Sets the current _environment_ value, which will be used when determining if environment-specific .env files will be loaded (for example `.env.production`),
...
- It _must_ be set to a simple reference to a single config item (for example `$APP_ENV`).
- We recommend setting this decorator only in your `.env.schema` file.
...
⚠️ Deprecated: use [`@currentEnv`](#currentenv) instead.
...
DB_HOST=localhost  # required (static value)
DB_PASSWORD=fnCall()  # required (function value)
DB_NAME=       # optional (no value)
DB_SCHEMA=''   # optional (empty string)

# @optional
OPTIONAL_ITEM=debug # optional (explicit)
...
- `inferFromPrefix(PREFIX)`: Item is marked not sensitive if key starts with the given `PREFIX`; all others are sensitive. Useful for marking keys with a prefix such as `PUBLIC_` as non-sensitive by default.
...
# ---
DB_HOST=localhost  # will be ignored
```
...
Listing keys as positional args (`@import(./.env.other, DB_HOST, DB_PORT)`) is **deprecated**; use `pick=[DB_HOST, DB_PORT]` instead. The positional form still works (exact-match only, no globs) but will warn.
:::
...
# @import(./.env.other, pick=[DB_HOST, DB_PORT])  # import only these keys (allowlist)
...
Injects multiple config values at once from an external data source. The first argument is a resolver that produces a string, for example a bulk resolver from a [secrets provider plugin](/plugins/overview/) such as [`opLoadEnvironment()`](/plugins/1password/#oploadenvironment) (1Password), [`infisicalBulk()`](/plugins/infisical/), or [`vaultSecret(…, raw=true)`](/plugins/hashicorp-vault/) (HashiCorp Vault). The string is parsed and injected as definitions within the file containing the decorator.

Bulk values participate in the normal file override chain: `process.env` still overrides everything, higher-precedence files (`.env.local`, `.env.production`, and other environment-specific files) override bulk values, and bulk values override schema-defined defaults in the same file. You control precedence by choosing which file to put `@setValuesBulk` in.
...
By default (neither `pick` nor `omit`) every key from the source is injected. You can't use `pick` and `omit` together. Both accept simple glob patterns: `*` matches any run of characters and `?` matches a single character (for example `pick=[API_*]`).
...
If a plugin doesn't exist for your provider, any CLI tool works via [`exec()`](/reference/functions/#exec), for example `@setValuesBulk(exec("my-secrets-cli export --format=json"), format=json)`.

:::note
When using `format=env`, function calls like `$VAR` references in unquoted values are not supported and will cause an error. Use single quotes for literal `$` signs (for example `'$LITERAL'`) or use `format=json` instead.
:::
...
This is primarily relevant for serverless deployments (such as Vercel and Netlify) where varlock injects resolved env data into the build output. The blob only appears in server-side code and is generally safe, but encryption provides extra protection, particularly against secrets leaking via sourcemaps.
...
Excludes directories from the [`varlock audit`](/reference/cli/encryption/#audit) code scanner. Paths are relative to the file containing the decorator. _Can be called multiple times; paths are merged additively._
...
The package name defaults to the output directory (for example `path=env/env.go` → `package env`); override it with `package=`.
</div>
...
Defaults to a global `final class Env`. For Composer/PSR-4 projects, set `namespace=` and/or `class=` (for example `@generatePhpEnv(path=src/Env.php, namespace="App\\Config", class=AppEnv)`).
</div>

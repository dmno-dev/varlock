...
export const DATA_TYPES: Array<DataTypeInfo> = [
  ...
  },
  {
    ...
    optionSnippets: [
      ...
      { name: 'allowedDomains', insertText: 'allowedDomains=[${1:example.com}]', documentation: 'Restrict the URL host to an allowed domain list. Hosts are matched in full, not as substrings. A comma-separated string is also accepted.' },
      { name: 'noTrailingSlash', insertText: `noTrailingSlash=${booleanChoiceSnippet()}`, documentation: 'Disallow a trailing slash on the URL path (root `/` is allowed).' },

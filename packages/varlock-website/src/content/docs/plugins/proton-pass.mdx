...
In production/CI, use a fully non-interactive login. The plugin supports two
...
If `pass-cli` is already logged in, you can also omit credentials and rely on the existing session.

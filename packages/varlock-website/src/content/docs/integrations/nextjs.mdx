...
By default, dynamic behavior follows sensitivity (`@defaultDynamic=inferFromSensitive`), but you can override per item with [`@dynamic`](/reference/item-decorators/#dynamic) / [`@static`](/reference/item-decorators/#static).

Marking a value `@dynamic` needs no extra code: server-side reads work as usual. The endpoint + hydration shown below are only needed when **browser code** reads a public+dynamic value (it must be fetched from the backend), which is a niche situation. See [When do you need the client-side machinery?](/guides/dynamic-config/#when-do-you-need-the-client-side-machinery). On Vercel in particular, env changes trigger a redeploy (and rebuild), so dynamic+public mainly matters for self-hosted deployments or promoted build artifacts.
...
Public+dynamic values are not inlined into client bundles, so browser code has to fetch them from your server: a route handler returning `getPublicDynamicEnv()` plus a one-time `loadPublicDynamicEnv()` call. This is rarely needed; see the [Next.js recipe in the guide](/guides/dynamic-config/#framework-recipes) for the full pattern and caveats.

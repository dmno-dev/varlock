...
# @required=eq($DEPLOY_TARGET, kubernetes)
REQUIRED_FOR_KUBERNETES_ITEM=
```
</div>
...
Sets whether the item is considered _sensitive_ - meaning it cannot be exposed to the public. The value is always redacted in CLI output, and client integrations can take further action to prevent leaks.
...
- `enabled` (`boolean`, default `true`): whether the item is sensitive. Lets you keep options while toggling sensitivity, including dynamically via a function (for example `enabled=forEnv(production)`). `@sensitive={enabled=false}` is equivalent to `@sensitive=false`.
...
Leak detection is a safety net that stops a secret from accidentally being sent in a response. Setting `preventLeaks=false` removes that net for this item, so only use it when a secret is _deliberately_ meant to leave the system (for example an endpoint forwarding a token to another service), never only to silence a leak-detection error. Prefer disabling it for a single item like this over turning off [`@preventLeaks`](/reference/root-decorators/#preventleaks) globally.
:::
</div>
...
Because internal items still resolve and validate, marking a variable internal does **not** produce an error if your application actually reads it. The value won't be in the environment at runtime. If your app uses one of these credentials directly (for example to write secrets back, renew Vault leases, or fetch more secrets at runtime), set `@internal=false` so it keeps being injected.
:::
...
Sets whether the item is _dynamic_ - meaning integrations must not replace it at build time and instead resolve it at runtime.
...
Marks an item as deprecated. Does not affect resolution or validation: the item still resolves and stays valid. Use this when a variable must keep working during a migration, but callers need to move off it.
...
- Pretty CLI output (for example `varlock load`): the key is struck through and a deprecated badge is shown
...
Sets an explicit placeholder string for the item, used in place of the real value when the item is routed through the [credential proxy](/guides/proxy/). A placeholder is what an untrusted child process (for example an AI agent) sees instead of the secret.

Without an explicit `@placeholder`, varlock derives one from the item's [`@type`](#type) format, falling back to a generic value. Set this when a generic placeholder would fail a client's key-format validation (for example an `sk-…` prefix check).
...
| `path` | Restrict to matching URL paths (glob), for example `"/v1/**"`. |
| `method` | Restrict to one or more HTTP methods, for example `[GET, POST]`. |
...
| `keys` | Array of additional item names to inject for this rule, for example `keys=[OTHER_KEY]`. |
| `substituteIn` | Where the placeholder may be swapped for the real value: `header` (default), `header:<name>`, `path`, `query`, `query:<param>`, or `body:<path>`, for example `[header, "body:client_secret"]`. Body always requires a path (`body:*` allows anywhere, for bodies that can't be parsed into a path). A placeholder anywhere no target allows blocks the request instead of substituting. See [Substitution surface](/guides/proxy/rules/#substitution-surface). |
...
| `rules` | Array of policy refinements sharing this rule's `domain`, for example `rules=[{path="/v1/**", block=true}]`. Each entry may set `path`/`method`/`block`/`approval`/`substituteIn`/`maxOccurrences` (not `domain`/`keys`) and injects nothing on its own. See the [Grouping rules guide](/guides/proxy/rules/#grouping-rules-for-one-domain). |

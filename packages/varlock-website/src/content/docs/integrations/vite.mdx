...
1. **Enable the Vite config plugin**
    ...
    export default defineConfig({
      plugins: [ /* plugin1, plugin2, ... */ varlockVitePlugin() ]
    });
    ```

    :::caution[Plugin ordering]
    In many cases, plugin ordering does not matter, but `varlockVitePlugin` must come _after_ any plugins which do parsing of non-standard files into JavaScript, such as `@vitejs/plugin-vue`.
    :::
</Steps>

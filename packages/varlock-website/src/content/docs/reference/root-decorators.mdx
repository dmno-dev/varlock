...
The default applies per file, so an item defined only in a file that does not set `@defaultRequired` itself, such as a `.env.local` or an [imported](/guides/import/) file, is required even when the root schema sets one. Mark those items with `@optional`, or set `@defaultRequired` in that file.

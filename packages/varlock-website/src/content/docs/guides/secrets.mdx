...
Varlock uses the term _sensitive_ to describe any value that must not be exposed to the outside world. This includes secret API keys, passwords, and other generally sensitive information. Instead of relying on prefixes (for example `NEXT_PUBLIC_`) to know which items may be "public", varlock relies on `@decorators` to mark sensitive items explicitly.
...
These decorators are normally used only in your `.env.schema` file, but may be used in any file, which can be useful to override sensitivity in a specific environment.

## Local encryption via `varlock()` ||local-encryption||

For local git-ignored secrets (overrides in `.env.local`), use the [`varlock()` function](/reference/functions/#varlock) to store encrypted values on disk.
...
# @sensitive @required
STRIPE_SECRET_KEY=op(op://dev-vault/stripe/api-key)
```
...
- Platform-specific features (for example biometric unlock for 1Password)
...
# @sensitive @required
API_CREDENTIAL=exec(`./scripts/fetch-secret.sh prod/api-credential`)
```
...
The varlock CLI itself redacts sensitive values in its own output. Commands like [`varlock explain`](/reference/cli/load-and-run/#explain) will display resolved values with partial masking (for example `my▒▒▒▒▒`) so you can verify configuration without exposing secrets in your terminal scrollback.
...
varlock reveal STRIPE_SECRET_KEY    # reveal a specific variable
varlock reveal STRIPE_SECRET_KEY --copy  # copy to clipboard (auto-clears after 10s)
```
...
When the output of [`varlock run`](/reference/cli/load-and-run/#run) is piped or redirected (CI logs, files, `| tee`, or any other non-TTY destination), the child process's stdout/stderr is piped through the same redaction engine, so any sensitive values that end up in your application's output will be masked automatically before they persist anywhere.

When output is attached to an interactive terminal, it passes straight through instead. Piping would break interactive tools (for example `psql`, `claude`) that rely on TTY detection, and a human at the terminal already has access to the secrets. Use `--no-redact-stdout` to force-disable redaction for piped output, or `--redact-stdout` to force it (for example to override `@redactLogs=false`; errors if output is attached to an interactive terminal, where redaction is not possible without breaking TTY behavior):
...
The same override can be set with the [`_VARLOCK_REDACT_STDOUT`](/reference/reserved-variables/#_varlock_redact_stdout) environment variable (`true`/`1` or `false`/`0`) when you can't change the command, for example in a wrapper script or CI config. The CLI flag takes precedence over the env var.
...
- **Global `Response` constructor**: intercepts the `Response` class used in edge runtimes (for example Cloudflare Workers), scanning bodies passed to the constructor and `Response.json()`
...
It can also be used to scan build output as an extra step to prevent accidentally bundling secrets into client-facing code. Most of our [drop-in integrations](/integrations/overview/) do this automatically, but this can be useful in some scenarios.
...
:::note
If varlock is installed as a project dependency (rather than a standalone binary), the generated hook command will automatically be prefixed with your package manager's exec command (for example `npx varlock scan` or `bunx varlock scan`).
:::
...
:::tip
If you already have an existing pre-commit hook, add `varlock scan` as an additional line in the script. It will exit with a non-zero code if any secrets are found, which will abort the commit.
:::

...
| Option | Meaning |
|---|---|
| `scheme` | **(required)** `hmac-sha256` or `hmac-sha512`. |
...
Signing schemes that recompute an existing signature (AWS SigV4, where the SDK signs with placeholder credentials and the proxy re-signs with the real key) are planned as additional `scheme` values on the same option.

...
| --- | --- |
| Vitest, parent and workers | `VITEST` and `NODE_ENV=test` |
| Jest, worker-scoped code (`setupFiles`, `setupFilesAfterEnv`, test files, app code under test) | `JEST_WORKER_ID` and `NODE_ENV=test` |
| Jest, parent-scoped code (`jest.config.*`, `globalSetup`, `globalTeardown`) | `NODE_ENV=test` only |
...
:::caution[When detection does not fire]
**Runners that set no signal.** `node --test` and Mocha set nothing varlock can read, so `VARLOCK_ENV` never resolves to `test` under them.

**An inherited `NODE_ENV`.** Test runners set `NODE_ENV=test` only when it is unset, so a value already present in your shell or CI survives. Anything in a `NODE_ENV=test` only row above is then left with no signal.

**Preloading varlock ahead of the runner.** With `node --import varlock/auto-load` or `NODE_OPTIONS`, varlock resolves in the parent process before the runner has set anything.

Whenever varlock resolves in a parent process, its values are injected as `__VARLOCK_ENV` and reused by the worker processes. A wrong result there propagates to every worker, even ones that would have detected `test` on their own.

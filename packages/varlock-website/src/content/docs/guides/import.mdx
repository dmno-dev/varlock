...
The [`@import()` root decorator](/reference/root-decorators/#import) allows you to import schema and/or values from other sources (currently only `.env` files), so you can share config across services within a monorepo, split up large schemas, or reuse pre-defined schemas. Multiple `@import()` calls may be used, and an imported source may itself import more sources.
...
For now, all imported files must be `.env` files (and may contain @env-spec decorators), but in the future, we may also support other formats (for example JSON or YAML) or even JS/TS files.
...
A registry (such as npm or jsr), package name, version, and path can be used to import a schema from a public package.
...
```env-spec
# @import(./.env.imported, pick=[DB_HOST, DB_PORT])   # allowlist
...
Older docs and configs list keys as positional args (`@import(./.env.imported, DB_HOST, DB_PORT)`). This still works but is deprecated and will warn. Use `pick=[DB_HOST, DB_PORT]` instead.
:::
...
The common case would be taking schema info from `.env.schema` and overriding a value from another source (for example `.env.local` or `.env.production`),
but there are many cases where root decorators, item decorators, and descriptions may be merged as well.

To do this, we walk our data sources in decreasing order of precedence, until we find something defined for the value or decorator we are evaluating.
...
## More details
- Root decorators that affect individual items (for example `@defaultRequired`) affect only the items that are defined in the file, not those in imported files
- An item with no value at all (for example `ITEM=`) will be skipped when looking for a value / function to use, but its presence can be used to add other decorators/description to the item
...
- Import the full directory (omit the key list) if the sub-package must inherit the parent's `@currentEnv` handling

### Circular imports

A circular chain of imports (for example `a` imports `b` which imports `a`) is not allowed. Varlock detects the cycle and fails with an explicit error showing the chain:

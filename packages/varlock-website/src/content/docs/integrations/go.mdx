...
:::note
Enums are generated as plain `string` (Go has no enum type), and keys whose names aren't valid identifiers (they contain `.` or `-`) are omitted from `Env` — read those from `os.Getenv`. See [Other languages](/integrations/other-languages/) for the shared details on the injected blob, raw env vars, and encryption.
:::

...
Redaction replaces a sensitive value wherever it appears, and it can't tell a leaked secret from ordinary text that happens to match. Varlock therefore checks that a value marked sensitive can actually be protected that way:

- Under 12 characters: a warning. The value is short enough to collide with normal content, so redacting it will also rewrite legitimate text in logs and proxied responses. Set `@sensitive=false` when it isn't really a secret, like an org slug or an account name. Some secrets are short by nature (a one-time code, a PIN) and can't be lengthened; acknowledge the collision risk with `@sensitive={allowShortValue=true}` to silence the warning without changing how the value is redacted.
- Under 3 characters: an error, and the only rule here that `@sensitive={allowShortValue=true}` cannot opt out of. That flag acknowledges a collision risk, and there is no output in which redacting one or two characters is merely risky.
- A boolean value: a boolean holds one bit, so there is no secret in it to protect, while redacting it would rewrite every `true`/`false` in logs and proxied response bodies. An explicit `@sensitive` on one is an error; a boolean sensitive only because of `@defaultSensitive` gets a warning. Either way the item stays sensitive for now, since demoting it would also make it [`@static`](#static) and change whether its value is inlined into a build. That demotion is planned for a future breaking release.
- A number value: redaction only ever replaces strings, so a number is never actually masked. An explicit `@sensitive` on one is an error, since it claims a protection that isn't being applied. A number that is sensitive only because of `@defaultSensitive` gets a warning instead and stays sensitive, so a hand-written schema doesn't fail over its ports and timeouts. Either way the fix is to make it a string, with `@type=string` or by quoting the value (`PIN="007123"`) - which also keeps leading zeros and precision past 2^53 that a number would drop.
- The [`@currentEnv`](/reference/root-decorators/#currentenv) item: the current env name drives conditional imports and `forEnv()`, and is echoed by most tools in the stack, so it is never a secret. An explicit `@sensitive` on it is an error; one that came from `@defaultSensitive` gets a warning.
- A value that appears inside a non-sensitive item's value: a warning, reported on the non-sensitive item. If the sensitive value is 12 characters or more, that item is carrying a real secret into logs, generated types, and client bundles where none of the sensitive item's protections apply. Below that, the problem runs the other way: redacting the short value will rewrite the containing value everywhere it appears. It fires even when `allowShortValue` is set, because here the collision is confirmed rather than hypothetical. This one is a warning rather than an error for now, since it is the only rule that depends on how two resolved values relate; it will become an error in a future breaking release.

Composite values (arrays, objects) are measured per element, since each element is registered for redaction on its own. An element that isn't a string is never redacted at all, so a sensitive `array(number)` or `record(number)` follows the same rule as a bare number.

The length rules apply the same way however the item became sensitive, because redaction registers the value either way. The rest are an error only when you wrote `@sensitive` on the item yourself, and a warning when `@defaultSensitive=true` swept it in: that default is what a hand-written schema inherits, so it picks up every port, flag and env name in the file, and failing those would be a bigger break than the problem warrants. Either way the fix is the same, `@sensitive=false` (or `@public`). Builtin `VARLOCK_*` variables are exempt from all of it.

Nothing here changes an item's sensitivity. Making a value public would also make it `@static` and eligible to be inlined into a build, so varlock reports the problem and leaves the decision to you.
...
- `allowShortValue` (`boolean`, default `false`): when `true`, silences the short-value warning and error described above, for secrets that are legitimately short and can't be lengthened (a one-time code, a PIN). Redaction is unchanged; this only records that the collision risk was read and accepted.
...
`varlock run` strips internal items from the child process environment **even if they were set in the ambient environment** (e.g. `OP_TOKEN=xxx varlock run ...`), so a secret-zero token never leaks into your app. If a _nested_ `varlock run` needs the internal value for its own resolution, pass [`--include-internal`](/reference/cli/load-and-run/#run) to keep it in the child env.

`varlock load --format json-full` excludes internal items by default too, since that format is commonly consumed programmatically (framework integrations shell out to this exact command to get their injected config) - pass [`--include-internal`](/reference/cli/load-and-run/#load) there for local debugging.
</div>
...
Suppresses "unused in schema" warnings from [`varlock audit`](/reference/cli/encryption/#audit) for this item. Useful for items that are only consumed by external tools and won't appear in your application code.

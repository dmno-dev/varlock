...
# @import(../../../.env.schema, pick=[DEPLOY_ENV, AWS_REGION])
# ---
MY_SERVICE_URL=...
```
...
- Import the full schema (omit the key list) if the sub-package should inherit the parent's `@currentEnv` handling
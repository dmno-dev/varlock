...
- A path matches that one directory. It uses the same prefixes as [`@import()`](#import): `./` or `../` relative to the scanned directory, `~/` for your home directory, or an absolute path. `./apps/docs` leaves a `docs` elsewhere in the tree alone, and `./fixtures` skips the top-level one but not `src/deep/fixtures`.

Two mistakes are reported rather than quietly matching nothing: a path written without one of those prefixes (`apps/docs` tells you to write `./apps/docs`), and a path that resolves outside the directory being scanned. Trailing separators are ignored, and `\` works as a separator so Windows-style entries are fine.

...
export const ROOT_DECORATORS: Array<DecoratorInfo> = [
  ...
  {
    name: 'generatePythonTypes',
    scope: 'root',
    summary: 'Generates Python types (TypedDict) from the schema.',
    documentation: 'Example: `# @generatePythonTypes(path=./env_types.py)`.',
    insertText: '@generatePythonTypes(path=${1:./env_types.py})',
    isFunction: true,
  },
  {
    name: 'generateRustTypes',
    scope: 'root',
    summary: 'Generates Rust types from the schema.',
    documentation: 'Example: `# @generateRustTypes(path=./src/env_types.rs)`.',
    insertText: '@generateRustTypes(path=${1:./src/env_types.rs})',
    isFunction: true,
  },
  {
    name: 'generateGoTypes',
    scope: 'root',
    summary: 'Generates Go types from the schema.',
    documentation: 'Example: `# @generateGoTypes(path=./env_types.go)`.',
    insertText: '@generateGoTypes(path=${1:./env_types.go})',
    isFunction: true,
  },
  {
    name: 'generatePhpTypes',
    scope: 'root',
    summary: 'Generates PHP types (phpstan) from the schema.',
    documentation: 'Example: `# @generatePhpTypes(path=./env_types.php)`.',
    insertText: '@generatePhpTypes(path=${1:./env_types.php})',
    isFunction: true,
  },
  {
    name: 'generateTypes',
    scope: 'root',
    summary: 'Deprecated — use @generateTsTypes (or another @generate*Types decorator).',

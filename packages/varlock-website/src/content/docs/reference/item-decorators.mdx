...
| `transform` | Request signing config, e.g. `transform={scheme="hmac-sha256", stringToSign="{timestamp}{method}{path}{body}", signatureHeader="X-Signature"}`. The proxy signs the final outbound request with the item's real value, which is consumed by the signer and never sent. Built-in schemes: `hmac-sha256`, `hmac-sha512`, and `http-basic` (which composes the `Authorization: Basic` header the proxy sends, since base64 hides a placeholder from substitution); plugins add more, e.g. `aws-sigv4`. Credential items are always named with `$ITEM` references (`secretKey=$PARTNER_SECRET`, or `username`/`password` for `http-basic`), never inline values; on an attached rule the decorated item supplies the credential by default. See [Request signing](/guides/proxy/rules/#request-signing) for per-scheme options. |

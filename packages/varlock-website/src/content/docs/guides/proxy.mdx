...
- **Response scrubbing is best-effort.** Responses are scrubbed back to placeholders only for **small, uncompressed text** bodies. **Compressed (gzip/br) or large (>2&nbsp;MB) response bodies are passed through unscrubbed**: the proxy can't scan into them. This only matters if an upstream *reflects your secret back in its response* (uncommon); the primary protection (the agent only ever holds a placeholder) is unaffected. Scrubbing is also scoped to the values a matching rule sends to that host, so a secret bound to a different route is left alone rather than scanned for everywhere.
- **Short secrets are a poor fit for scrubbing.** Scrubbing replaces the real value wherever it occurs in a response, and it can't tell a reflected credential from ordinary text that happens to match. A short value (an org slug, an account id, a dev password) will be rewritten in legitimate content, so `varlock proxy` warns at start when a proxied secret is short. Mark it [`@sensitive=false`](/reference/item-decorators/#sensitive) when it isn't really a secret.

...
Code-generation decorators turn your schema into generated code, and [plugins](/guides/plugins/) can register generators that emit other kinds of code. See the [Code generation guide](/guides/code-generation/) for the full picture. Varlock ships these built-in generators — add one decorator per output file you want:

- [`@generateTsTypes()`](#generatetstypes) — TypeScript type declarations (augments `varlock/env` / `process.env`)
- `@generatePythonEnv()` — a Python env module (typed values + loader + `SENSITIVE_KEYS`)
- `@generateRustEnv()` — a Rust env module (serde struct + loader)
- `@generateGoEnv()` — a Go env module (struct + `Load()`)
- `@generatePhpEnv()` — a PHP env module (class + loader)

Each of these is a root decorator that _can be called multiple times_. They share these common args:

- `path`: Relative filepath to write the generated file to.
- `auto`: Controls whether generation runs automatically on every load (defaults to `true`). Set to `false` to disable automatic generation and instead run [`varlock typegen`](/reference/cli-commands/#typegen) explicitly.
...
:::note[Disabling automatic generation]
If you prefer to generate manually rather than on every load, set `auto=false` and run [`varlock typegen`](/reference/cli-commands/#typegen) explicitly:

```env-spec
# @generateTsTypes(path=./env.d.ts, auto=false)
```

This is useful when you want full control over when files are regenerated, for example in a CI pipeline or a dedicated build step.
:::

#### `@generateTsTypes()`

In addition to the common args above, the TypeScript generator accepts:

- `env`: how the coerced `ENV` object is exposed (defaults to `augment`).
  - `augment` — globally augment `varlock/env` so `import { ENV } from 'varlock/env'` is typed.
  - `module` — emit a package-local importable `ENV` (typed to _this_ package's schema) with **no** global augmentation. Use this in monorepos where multiple packages have different schemas. Requires a `.ts` (or `.js`) output path, and you import from the generated file (e.g. `import { ENV } from './env'`).
  - `none` — emit only the type definitions, no `ENV` binding.
- `processEnv`: how `process.env` is typed — `strict`, `loose`, or `none` (defaults to `strict`, but to `none` when [`@disableProcessEnvInjection`](#disableprocessenvinjection) is set, since values aren't put on `process.env` then). Note that `@types/node` declares a base string index signature that can't be removed, so extra keys remain allowed on `process.env` regardless.
- `importMetaEnv`: how `import.meta.env` is typed — `strict`, `loose`, or `none` (defaults to `strict`).

```env-spec
# monorepo-friendly: importable ENV, no globals
# @generateTsTypes(path=./env.ts, env=module, processEnv=none, importMetaEnv=none)
```

:::note[Deprecated: `@generateTypes()`]
`@generateTypes(lang=ts, path=...)` still works as a TypeScript-only alias for `@generateTsTypes`, but is deprecated. For other languages, use the matching `@generate*Env` decorator (`@generateTypes(lang=py, ...)` will error with a pointer to `@generatePythonEnv`).
:::

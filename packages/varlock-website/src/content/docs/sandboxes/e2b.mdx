...
(Running the orchestrator under `varlock run -- node orchestrate.ts` works too, and is the option when the orchestrator is not a Node program.)

Rather than enumerating keys, you can forward varlock's own serialized env blob, so there is one variable to pass no matter how many secrets are involved:

```ts title="orchestrate.ts"
const sandbox = await Sandbox.create({
  envs: { __VARLOCK_ENV: process.env.__VARLOCK_ENV! },
});
```

Inside the sandbox, importing `varlock/env` unpacks it: no `.env` files, no varlock CLI, and no second resolve (so no plugin credentials in the sandbox either).

```js title="agent.js (in the sandbox)"
import { ENV } from 'varlock/env';

ENV.STRIPE_SECRET_KEY; // ✨ typed, and also re-injected into process.env
```

Scope the blob with [`--filter`](/reference/cli-commands/#common-options) so a sandbox only receives what it needs (`varlock run --filter="STRIPE_*" -- ...`). The sandbox needs `varlock` as a dependency for the import; to also get log redaction there, call [`patchGlobalConsole()`](/integrations/javascript/) from `varlock/patch-console`.
...
// yourself. It is the credential to USE the broker over the tunnel, not to
// read its secrets.
...
await broker.commands.run(
  `cd /home/user/proj && ${VARLOCK} proxy start --tunnel --port 8080 --cert-dir /home/user/proj/.varlock-ca --persist-ca > /home/user/proxy.log 2>&1`,
  {
    background: true,
    timeoutMs: 0,
    envs: {
...
# agents: varlock proxy run --url wss://abc123.ngrok.app --token $VARLOCK_PROXY_TOKEN -- <command>
```

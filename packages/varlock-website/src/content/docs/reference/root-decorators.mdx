...
Two mistakes are reported rather than quietly matching nothing: a path written without one of those prefixes (`apps/docs` tells you to write `./apps/docs`), and a path that resolves outside the project. Trailing separators are ignored, and `\` works as a separator so Windows-style entries are fine.

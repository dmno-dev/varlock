...
Call `load_env()` once, then read keys off the dict. Values are coerced (`int`, `bool`, and the other [data types](/reference/data-types/)), not raw strings:
...
`load_env()` raises a clear error if `__VARLOCK_ENV` is missing (for example, you forgot `varlock run`). Optional keys that are unset are absent from the dict (`NotRequired`), not present as `None`.
...
When using varlock, don't also load `.env` from Python (`python-dotenv`, Pydantic's `env_file=`, or any other dotenv loader) unless you intentionally want a second source of truth. Varlock is the single resolver: your schema, plugins, and environment files all flow through it before Python starts.
:::

...
Redaction replaces a sensitive value wherever it appears, and it can't tell a leaked secret from ordinary text that happens to match. Varlock therefore warns when a sensitive value is short enough to collide with normal content (under 12 characters), because redacting it will also rewrite legitimate text in logs and proxied responses. Set `@sensitive=false` when the value isn't really a secret, like an org slug or an account name. Some secrets are short by nature (a one-time code, a PIN) and can't be lengthened; acknowledge the collision risk with `@sensitive={allowShortValue=true}` to silence the warning without changing how the value is redacted.

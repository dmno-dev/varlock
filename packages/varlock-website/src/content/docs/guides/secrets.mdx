...
Varlock scans outgoing HTTP responses at runtime to detect if any sensitive values are being accidentally sent to clients. If a leak is detected, varlock throws an error with a detailed diagnostic message including the config item key and where the leak was detected. The response itself is never delivered: varlock closes the connection, so the client sees a failed request rather than a leaked value. This applies to a leak caught part-way through a streamed response and to one caught on `ServerResponse.end` (for example Next.js Pages Router `res.json()`, which sends the whole body in one call). The error on your server log is the diagnostic, so check there when a request fails this way.

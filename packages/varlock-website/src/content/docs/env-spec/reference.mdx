...
@env-spec has no regex literal. A value like `/pattern/` is an ordinary string, so it follows the [normal unquoted value rules](#unquoted-values) and can be a path or any other text without special treatment.

Tools built on @env-spec take a pattern through a function call instead. In varlock that is `regex("pattern", "flags")`: the pattern is a quoted string, so commas, spaces and slashes inside it need no escaping, and flags are an optional second argument. See [`regex()`](/reference/functions#regex).

```env-spec
ITEM=fn(regex("^dev.*", "i"), dev, "production", prod)
...
Within a decorator literal (or function call), a `#` followed by a space (or by nothing)
introduces a comment that runs to the end of the line, so entries can be commented out or
annotated. A `#` directly followed by a letter or digit is a value, not a comment: this is
how `#tag` selectors are written (`pick=[#frontend]`).
...
#   #frontend,          ← a tag selector, kept
# ])
DATABASE_URL=
```

In item values (`KEY=fn([...])`), any `#` starts a comment, as in plain dotenv.

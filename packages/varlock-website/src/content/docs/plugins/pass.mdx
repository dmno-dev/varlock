...
Access multiple different password stores (for example personal and team):
...
- `multiline` (optional): if `true`, returns the full entry content instead of only the first line

...
Can be re-run on projects that already have a `.env.schema` to install or update agent tooling (skills and Docs MCP).
...
**Options:**
- `--agent`: Run non-interactively for agent/automation workflows. Skips confirmation prompts, uses deterministic defaults for schema generation, and installs agent tooling.
- `--ai-tool`: Override AI tool auto-detection. One of: `cursor`, `claude`, `codex`, `opencode`, `vscode`. Required in `--agent` mode when auto-detection fails.
- `--install-docs-mcp`: Install the [Varlock Docs MCP](/guides/mcp/#docs-mcp) server in the detected or overridden AI tool.
...
# Install agent tooling on an existing project
varlock init --agent --ai-tool cursor --install-docs-mcp
```

:::tip
See the [AI Tools guide](/guides/ai-tools/#agent-mode) for more on using `--agent` with AI coding assistants.
:::

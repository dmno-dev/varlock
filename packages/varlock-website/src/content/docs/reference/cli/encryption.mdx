...
- `--ignore` / `-i`: Directory to exclude from code scanning (can be specified multiple times). Accepts the same forms as [`@auditIgnorePaths()`](/reference/root-decorators/#auditignorepaths), which it is merged with: a bare name matches at any depth, an entry with a separator is anchored at the scan root.
...
# Exclude one specific directory, anchored at the scan root

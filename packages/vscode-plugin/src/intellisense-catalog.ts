...
export const DATA_TYPES: Array<DataTypeInfo> = [
  ...
  },
  {
    ...
    optionSnippets: [
      ...
      { name: 'allowedDomains', insertText: 'allowedDomains=[${1:example.com}]', documentation: 'Restrict the URL host to an allowed list. Matched in full, not as substrings. An entry without a port allows any port; add one to pin it. Entries are hosts only, with no scheme or path. A bare string is a single host.' },

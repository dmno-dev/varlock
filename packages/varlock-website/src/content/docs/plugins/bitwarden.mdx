...
varlock acquires and caches a CLI **session token** for you: on first load it runs `bw unlock` (prompting for your master password), then caches the token — encrypted, with a configurable TTL — and reuses it on subsequent loads. You no longer need to manually unlock and paste a session token into a `.env` file.
...
On the first `varlock load`, you'll be prompted for your master password once; the resulting session token is cached and reused until it expires (`sessionTtl`, default `15m`).
...
- `sessionTtl` (default `15m`) — how long the unlocked CLI session token is cached before varlock re-unlocks (prompting again).
...
- `sessionTtl`: how long the auto-unlocked session token is cached before re-unlock (default `"15m"`; e.g. `"1h"`, or `forever`).

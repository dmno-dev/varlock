...
### `_VARLOCK_ALLOW_ENV_SNAPSHOT_CONFLICTS`

When a server boots from a build-time baked env snapshot (e.g. a standalone build running in a container without the varlock CLI), runtime env values that conflict with the snapshot fail the boot loudly, since they cannot be validated or applied. Set to `1` / `true` to downgrade the failure to a logged warning and boot on the baked values; the conflicting runtime vars are ignored by `ENV` but left untouched in `process.env`. Prefer booting through [`varlock run`](/reference/cli/load-and-run/#run) instead, which re-resolves against the runtime environment. See the [Docker guide](/integrations/docker/).
...
A marker set when the runtime booted from a build-time baked env snapshot rather than a fresh resolution, so child/worker processes inheriting the [`__VARLOCK_ENV`](#__varlock_env) blob apply the same semantics (conflicting runtime env values fail the boot, see [`_VARLOCK_ALLOW_ENV_SNAPSHOT_CONFLICTS`](#_varlock_allow_baked_env_conflicts)).

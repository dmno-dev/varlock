...
- **Personal setup**: keep a schema in your home directory (for example `~/.env.claude`) and pass it with [`-p`](/reference/cli/load-and-run/#run) when launching a tool from any directory (for example via a shell alias).

...
SvelteKit uses Vite, so dynamic/public behavior comes from varlock core + the Vite integration. Use `@dynamic` for public values that should not be inlined into client bundles; server-side reads (`load` functions, endpoints) keep using `ENV.KEY` as usual, and prerendered routes should only use static values.
...
If you do need browser access, see the [SvelteKit recipe in the guide](/guides/dynamic-config/#framework-recipes).

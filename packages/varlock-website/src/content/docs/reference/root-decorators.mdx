...
Defines a named OAuth provider that [`oauth()`](/reference/functions/#oauth) items reference by id, so client config is written once and shared by every token minted from it. Can be declared multiple times with different ids.

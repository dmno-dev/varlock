...
Some frameworks use Vite under the hood, and some projects use Vite directly. Either way, often there is some [automatic loading of .env files](https://vite.dev/guide/env-and-mode.html) happening, but it is fairly limited. To integrate varlock into a Vite-powered application, you must use our [`@varlock/vite-integration`](https://npmx.dev/package/@varlock/vite-integration) package, which is a [Vite plugin](https://vite.dev/guide/using-plugins.html).
...
:::note[Astro users]
For [Astro](https://astro.build), which is also powered by Vite, use our [Astro integration](/integrations/astro/).
::::
...
By default, varlock loads `.env` files from the current working directory. If you store your `.env` files in a custom directory, you can configure this using the `varlock.loadPath` option in your `package.json`:
...
Point `loadPath` to a **directory** if you want varlock to automatically load all relevant files (`.env.schema`, `.env`, `.env.local`, and environment-specific files). Pointing to a specific file will only load that file and anything it explicitly imports via `@import`.
:::
...
:::caution[Don't use `resolved-env` on Cloudflare builds]
On Cloudflare Workers, production env is injected at runtime from bindings via [`varlock-wrangler`](/integrations/cloudflare/), not baked into the bundle. If Cloudflare is detected (via `@varlock/cloudflare-integration` or SvelteKit's `@sveltejs/adapter-cloudflare`) and `ssrInjectMode` is explicitly set to `'resolved-env'` for a production build, the plugin throws rather than silently shipping resolved (and possibly sensitive) values into the worker artifact for no reason. This only applies to builds. Some adapters (for example Astro's `@astrojs/cloudflare`) run SSR in workerd during their own dev server with no binding-injection hook available, so `resolved-env` is still used there by default.
:::
...
If you find you are not getting type completion on `ENV`, you may need to add your vite config and generated type files (`env.d.ts` by default) to your `tsconfig.json`'s `include` array.
:::
...
You can use [`varlock run`](/reference/cli/load-and-run/#run) to inject resolved config into other scripts as regular environment vars.
...
Varlock can load multiple _environment-specific_ `.env` files (for example `.env.development`, `.env.preview`, `.env.production`) by using the [`@currentEnv` root decorator](/reference/root-decorators/#currentenv). **This is different than Vite's default behaviour, which relies on its own [`MODE` flag](https://vite.dev/guide/env-and-mode.html#modes).**

...
The `@import()` decorator supports an optional `enabled` parameter that allows you to conditionally load env files based on boolean expressions. This is useful for environment-specific imports or feature flags.

The `enabled` parameter accepts:
- Static boolean values: `true` or `false`
- Dynamic expressions using functions like [`eq()`](/reference/functions/#eq), [`not()`](/reference/functions/#not), or [`forEnv()`](/reference/functions/#forenv)
- Any expression that evaluates to a boolean

When a variable is referenced in the `enabled` condition (e.g., `$ENV`), varlock automatically handles early resolution to ensure the variable's value is available before evaluating the condition.

**Examples:**

```env-spec
# Import different files based on environment
...
```env-spec
# Use with forEnv() for environment-based imports
# @currentEnv=$APP_ENV
# @import(./.env.development, enabled=forEnv("development"))
# @import(./.env.production, enabled=forEnv("production"))
# ---
APP_ENV=development
```
...
# @import(./.env.optional, enabled=not(eq($SKIP_OPTIONAL, "true")))
# ---
SKIP_OPTIONAL=false
```

If `enabled` is not specified, it defaults to `true`, meaning the import will always be processed.

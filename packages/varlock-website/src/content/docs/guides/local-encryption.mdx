...
Under the scope control sits one checkbox, **Cover anything this vault can open**, ticked by default:
...
The panel draws these as one row, ordered from least to most permissive: `Once`, `10min`, `1hr`, `Custom`, `This session`. Picking `Custom` reveals a field and a `min` / `hr` toggle under the row. Whatever you set shows on the rung itself, so the row still reads as an ordered list of answers, and selecting that rung again reopens the field to adjust it. Switching the unit converts the value rather than rereading the number, so 90 minutes becomes 1 hour and not 90 of them.

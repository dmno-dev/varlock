...
`varlock load --format json` emits HTML-script-safe JSON (it escapes `<` as `\u003C`), so a public value containing something like `</script>` cannot break out of the tag or inject markup. If you build the payload some other way, escape it yourself before embedding it in HTML.

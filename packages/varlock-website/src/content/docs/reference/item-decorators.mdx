...
| `substituteIn` | Where the placeholder may be swapped for the real value: `header` (default), `header:<name>`, `query`, `query:<param>`, or `body:<path>`, e.g. `[header, "body:client_secret"]`. Body always requires a path (no bare `body`). A placeholder anywhere no target allows blocks the request instead of substituting. See [Substitution surface](/guides/proxy/#substitution-surface). |

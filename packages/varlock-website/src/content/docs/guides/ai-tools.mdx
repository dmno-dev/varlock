...
Varlock provides a non-interactive `--agent` flag for `init` and `load` commands, designed for AI coding assistants running commands on your behalf.
...
- Uses deterministic defaults when multiple `.env.example` files are found
- Prints schema review guidance for the agent to follow
- Detects your AI tool and installs a project-scoped Varlock skill with security rules, schema checklists, and CLI guidance
- Updates the installed skill automatically when a newer bundled version is available
- Can optionally install the [Docs MCP](/guides/mcp/#docs-mcp) server

You can re-run init on projects that already have a `.env.schema` to install or update agent tooling:
...
**Flags:**
- `--ai-tool` — Override auto-detection (`cursor`, `claude`, `codex`, `opencode`, `vscode`). Required when auto-detection fails in agent mode.
- `--install-docs-mcp` — Install the Varlock Docs MCP server in the detected AI tool

In interactive mode (without `--agent`), init will prompt whether to install the Docs MCP after setup.

...
- The pattern may not contain a `,`, a space, or a `)`, since each of those ends a value. A pattern containing any of them must use `regex("pattern", "flags")` instead
...
# a `{n,m}` quantifier holds a comma, so this pattern needs regex()

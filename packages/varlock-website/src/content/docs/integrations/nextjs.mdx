...
1. **Verify the override is active**
    ...
    The baked env is a build-time snapshot, so values passed in the runtime environment (`docker run -e ...`) cannot be validated or applied to it. If the runtime env conflicts with the snapshot for a schema-declared key, varlock fails loudly with an error naming the keys instead of silently ignoring the runtime values. Self-hosted containers (e.g. `output: 'standalone'`) that pass env vars at boot should start the server through a `varlock run` entrypoint (see the [Docker guide](/integrations/docker/)), which re-resolves against the runtime environment with full validation and coercion. Blob-only deploys, where no schema values are set in the runtime env, boot from the snapshot as before.
    :::

...
An entry is either a directory name or a path, and it says which by whether it starts with `./`:

- A bare name matches **any** directory with that name, wherever it appears. `fixtures` skips both `./fixtures` and `./src/deep/fixtures`.
- A `./` entry is a **path relative to the scan root**, matching that one directory. `./apps/docs` leaves a `docs` elsewhere in the tree alone, and `./fixtures` skips the top-level one but not `src/deep/fixtures`.

A path written without the `./` is an error, rather than being guessed at: `apps/docs` tells you to write `./apps/docs`. Trailing separators are ignored, `/apps/docs` is accepted as equivalent to `./apps/docs`, and `\` works as a separator so Windows-style entries are fine.

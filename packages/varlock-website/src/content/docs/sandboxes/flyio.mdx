...
Egress is permissive by default: proxied requests to hosts without a rule pass through untouched, which is acceptable because agents hold only placeholders. To make the broker refuse anything that does not match a rule, set [`@proxyConfig={egress="strict"}`](/guides/proxy/rules/#egress-modes) in the schema header.
...
# doesn't invalidate agents that already trust it. --allow-reload lets you apply
# schema edits later without a restart; the reload channel is only reachable
# from inside the sprite, not by agents. Schema keys resolve from the process
# env, so the env map carries the bootstrap: usually just your plugin's
  ...
  -H 'Content-Type: application/json' -d @- <<EOF
{
  ...
  "args": [
    ...
    "--cert-dir", "/home/sprite/proj/.varlock-ca", "--persist-ca", "--allow-reload"
  ],
...
- **No code changes.** Substitution happens transparently at the wire, so SDKs, CLIs, and agent tools work unmodified. Connectors rewrite the base URL, so anything calling an API has to be changed to point at the gateway.

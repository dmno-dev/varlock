...
- **Zero-config authentication** - Provide your vault URL and authentication happens automatically
...
- **Auto-infer secret names** from environment variable names (for example `DATABASE_URL` → `database-url`)
...
- **[OIDC workload identity](/guides/oidc/)** - Authenticate using platform OIDC tokens (such as Vercel and GitHub Actions)
...
If you're deploying outside of Azure (for example AWS, GCP, or on-premises), wire up service principal credentials:
...
**Key/value args:**
- `vaultUrl` (required): Azure Key Vault URL (for example `https://my-vault.vault.azure.net/`)

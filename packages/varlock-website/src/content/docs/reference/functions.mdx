...
The `forEnv()` helper function is used in some decorators to dynamically set whether an item is required or optional based on the current environment.

**Usage:**
- `@required=forEnv(env1, env2)` - Item is required if current environment matches any of the specified environments
- `@optional=forEnv(env1, env2)` - Item is optional if current environment matches any of the specified environments

**Requirements:**
- Must be used with `@required` or `@optional` decorators
- Requires an `@envFlag` to be set in your `.env.schema` file
...
**Note:** This helper cannot be used as an item value - it's only for decorator arguments.
</div>
</div>

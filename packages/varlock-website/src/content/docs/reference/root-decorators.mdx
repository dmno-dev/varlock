...
Defines a **detached** [credential proxy](/guides/proxy/) rule: a domain-level policy (match or `block`). It injects no secret on its own, but can inject named items via `keys=[...]`, or sign matching requests via `transform={..., secretKey=$ITEM_NAME}` (the consumed secret is required on a detached rule since there is no attached item to default to, and is always written as a reference; see [Request signing](/guides/proxy/rules/#request-signing)).

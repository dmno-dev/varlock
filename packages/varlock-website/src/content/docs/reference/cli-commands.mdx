...
[`@internal`](/reference/item-decorators/#internal) items are always excluded, regardless of `--filter` — even if a selector would otherwise match them (e.g. by exact key name) — except where `load` already shows internal items for debugging/inspection: the default `pretty` format, `--format json-full`, and `--agent` (which shows them redacted). `--filter` only narrows those views further; it never causes internal items to appear anywhere they wouldn't otherwise. Everywhere else on `load` (plain `json`, `env`, `shell`) and on `run`'s injected env / `__VARLOCK_ENV` blob, internal items stay excluded no matter what `--filter` says — only `--include-internal` (on `run`) opts them back in, and even then they still have to satisfy `--filter` if one is set.

On `run`, `--filter` also strips excluded items from the `__VARLOCK_ENV` blob, so a filtered-out var can't leak to the child process via env inspection either.

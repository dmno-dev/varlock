...
Your enum needs to cover every value `$VARLOCK_ENV` can produce. If your environments do not line up, use [`remap()`](/reference/functions#remap) to translate them. Unmatched values pass through unchanged, so you only list what differs:
...
We do not recommend using `NODE_ENV` itself as your environment flag; see [`@currentEnv`](/reference/root-decorators/#currentenv).

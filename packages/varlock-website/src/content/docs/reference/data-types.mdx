...
- `allowedDomains` (string[]): List of allowed hostnames, matched in full and case-insensitively. A port on the value is ignored, so `[localhost]` accepts `http://localhost:3000`, and entries must not include one. A bare string is treated as a single hostname; list two or more as an array

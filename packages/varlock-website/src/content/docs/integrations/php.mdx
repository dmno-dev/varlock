...
:::note
Keys whose names aren't valid identifiers (they contain `.` or `-`) are omitted from `Env` — read those from `getenv()`. See [Other languages](/integrations/other-languages/) for the shared details on the injected blob, raw env vars, and encryption.
:::

...
If you keep these router options and also have prerendered routes, those routes ship a build-time snapshot and `hydrate()` fills the store with it, which makes a plain `loadPublicDynamicEnv()` skip its fetch. Pass `force: true` there and keep `ENV` reads out of render:

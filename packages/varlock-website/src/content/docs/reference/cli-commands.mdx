...
- `--agent` — the same flat map, but `@sensitive` values are redacted (e.g. `"su▒▒▒▒▒"`), so it is safe to print in logs or agent transcripts. Implies JSON output; not compatible with `--format env`/`shell`.
- `--format json-full` — the full serialized graph: top-level `basePath`, `sources`, `config` (per-item metadata including resolved value, validation state, and sensitivity), `settings`. Use this when you need per-item validation/error detail rather than just values.
...
**Options:**
- `--env`:         Resolve in the context of a specific environment (e.g., `--env production`)
- `--path` / `-p`: Path to a specific `.env` file or directory to use as the entry point. Can be specified multiple times to load from multiple paths — later paths take higher precedence.
...
- `--key-id`:  Encryption key ID to use (defaults to `varlock-default`)

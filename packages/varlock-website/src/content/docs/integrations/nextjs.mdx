...
Note that in the browser, reading a dynamic+public key *before* `loadPublicDynamicEnv()` completes returns `undefined`. Gate rendering on the load completing, or handle the `undefined` initial state.

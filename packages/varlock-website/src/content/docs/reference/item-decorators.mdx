...
| `transform` | Request signing config, e.g. `transform={scheme="hmac-sha256", stringToSign="{timestamp}{method}{path}{body}", signatureHeader="X-Signature"}`. The proxy computes an HMAC over the final outbound request with the item's real value (which is consumed by the signer and never sent) and writes it into the named header. `secretKey` defaults to the decorated item on an attached rule and is required on a detached one. See [Request signing](/guides/proxy/rules/#request-signing) for all options. |

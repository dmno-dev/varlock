...
Prerendered routes run `dehydrate()` at build time, so their HTML carries a build-time snapshot and `hydrate()` restores it in the browser. A plain `loadPublicDynamicEnv()` call will not fix that: it sees the store as already hydrated and skips the fetch. Either leave these router options out and use the endpoint recipe everywhere, or keep them and have prerendered routes refetch explicitly:
...
function PrerenderedPage() {
  // updating the store does not re-render on its own, so copy the value into state
  const [runtimeFlag, setRuntimeFlag] = useState(ENV.RUNTIME_FLAG);
  useEffect(() => {
    // force: the store already holds the build-time snapshot
    loadPublicDynamicEnv({ endpoint: '/api/public-env', force: true })
      .then(() => setRuntimeFlag(ENV.RUNTIME_FLAG));
  }, []);
  return <p>{runtimeFlag}</p>;
}
```

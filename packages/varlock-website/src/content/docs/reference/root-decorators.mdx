...
<div>
### `@proxy()`
**Arg types:** `(domain: string, path?, method?, block?, approval?, ...)`

Defines a **detached** [credential proxy](/guides/proxy/) rule — a domain-level policy (match, `block`, or `approval`) that injects **no** secret. Requires [`@enableProxy`](#enableproxy).

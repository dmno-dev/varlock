...
- ❌ `process.env.__VARLOCK_ENV is not set` -
  This error appears when the plugin has been set up, but the `@next/env` override has not been set up properly.
...
- ❌ `Property 'SOMEVAR' does not exist on type 'TypedEnvSchema'` -
  If you are sure the item does exist in your schema, make sure the [`@generateTypes` root decorator](/reference/root-decorators/#generatetypes) is enabled, and that the path to the generated types file is included in your `tsconfig.json`.

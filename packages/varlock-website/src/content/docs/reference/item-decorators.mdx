...
- `allowShortValue` (`boolean`, default `false`): when `true`, silences the [short-value warning](/guides/secrets/#values-varlock-cant-protect) for secrets that are legitimately short and can't be lengthened (a one-time code, a PIN). Redaction is unchanged; this only records that the collision risk was read and accepted. It does not apply to values under 3 characters, which are an error either way.

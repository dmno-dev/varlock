...
Manages [credential proxy](/guides/proxy/) sessions — running an untrusted child process so it only sees placeholder secrets while real values are injected at the network boundary. Requires [`@enableProxy`](/reference/root-decorators/#enableproxy) in your schema.
...
- `start`: Start a long-lived proxy session that owns the terminal (interactive approval prompts and a live request log appear here). Stop with `Ctrl+C`.
- `rules`: Print a static summary of the effective `@proxy` configuration — the rules (host/path/method, block/approval) and each secret's mode (proxied / placeholder / passthrough / omit) — without starting a proxy.
...
- `refresh`: Hot-reload a running `proxy start` daemon after an intentional schema edit — re-resolves and swaps the live policy (rules, injected secrets, egress) without restarting.
- `stop`: Stop a session.
...
- `--no-redact-stdout`: For `run`, disable stdout/stderr redaction (full TTY pass-through).
...
# Daemon in one terminal, attach from another (enables interactive approvals)
varlock proxy start

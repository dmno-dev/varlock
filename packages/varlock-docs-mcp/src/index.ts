  ...
  async init() {
    ...
    this.server.tool('varlock docs', {
      ...
      const docs = await env.AI.autorag('varlock-docs-mcp-search').aiSearch({
        query,
        system_prompt: "You are a helpful assistant that can answer questions about the Varlock docs. Don't make up information, only answer questions that are based on the actual content of the docs.",
        rewrite_query: true,
      });
      return {

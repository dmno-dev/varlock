...
If you have installed varlock as a project dependency instead of globally, run this via your package manager:
...
Varlock can load multiple environment-specific `.env` files (for example `.env.development`, `.env.production`) by using the [`@currentEnv` root decorator](/reference/root-decorators/#currentenv) to specify which env var will set the current environment.

If not using the default (`development`, unless your schema sets a different one), pass it in as an environment variable when running your command.

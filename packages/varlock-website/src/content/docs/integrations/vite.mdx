...
With `resolved-env`, the build artifact carries its own config and the baked values stay authoritative at runtime: booting through [`varlock run`](/reference/cli/load-and-run/#run) does not replace them. If the runtime environment provides a different value for a schema-declared key, varlock logs a loud warning naming the keys and boots on the baked values, since runtime values cannot be validated or applied to a baked snapshot. To have runtime env apply, rebuild with the values set, or use a non-baking mode (`init-only` / `auto-load`).
...
Use `@dynamic` for public values that should not be inlined into client bundles:

...
No endpoint needed. TanStack Router calls `dehydrate()` on the server during SSR and awaits `hydrate()` in the browser before the first render, so the values ride along with the router's dehydrated state and `ENV.KEY` reads work in components from the initial render, with no hydration mismatch:
...
Prerendered routes run `dehydrate()` at build time, so their HTML carries a build-time snapshot and `hydrate()` restores it in the browser. Reading `ENV.KEY` while a route prerenders throws (the build-time guard, see above), and a plain `loadPublicDynamicEnv()` call in the browser will not help: it sees the store as already hydrated and skips the fetch. Either leave these router options out and use the endpoint recipe everywhere, or keep them and have prerendered routes fetch explicitly, without reading `ENV` during render:
...
function PrerenderedPage() {
  ...
  useEffect(() => {
    // force: the store already holds the build-time snapshot
...
with `/api/public-env` being a server route that returns `getPublicDynamicEnv()`, as in the other recipes.

...
Entries are matched as directory **names**, so every directory with that name is skipped wherever it appears in the tree. A leading `./` and a trailing slash are ignored, so `fixtures`, `./fixtures` and `./fixtures/` are equivalent. Multi-segment paths like `generated/config` match nothing: name a single directory (`config`) or a parent that contains it (`generated`).

```env-spec
# @auditIgnorePaths(fixtures, generated)
# ---
API_KEY=
```

```env-spec
# called multiple times, names are merged
# @auditIgnorePaths(fixtures)
# @auditIgnorePaths(generated, e2e)
# ---
API_KEY=
```

These are merged with any `--ignore` names passed on the command line. `.git`, `node_modules`, `dist`, `build`, `.next`, `vendor` and `.venv` are always skipped, so they don't need listing.
</div>

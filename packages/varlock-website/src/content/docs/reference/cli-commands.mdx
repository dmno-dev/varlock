...
Regenerates TypeScript type definitions (`.d.ts` files) from your `.env.schema` without running validation or producing output. Requires the [`@generateTypes` root decorator](/reference/root-decorators/#generatetypes) in your schema.

Useful for editors, TypeScript, and CI when you need type generation separate from validation. Succeeds silently on success.
...
**Examples:**
```bash
# Regenerate types (silent on success)
varlock typegen

# Use a specific entry point
varlock typegen --path .env.prod

# Use a specific environment
varlock typegen --env production
```

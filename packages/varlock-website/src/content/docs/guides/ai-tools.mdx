...
- **`varlock load --format json-full`** — the full serialized graph (sources, per-item validation state, sensitivity) when you need to reason about *why* something is invalid, not just the values.

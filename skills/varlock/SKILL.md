...
This skill covers managing env vars and secrets in a project with varlock.

> **Docs**: https://varlock.dev (every page is also available as markdown: send `Accept: text/markdown`)
> **Docs MCP**: https://docs.mcp.varlock.dev/mcp (use it if installed in your AI tool)
> **Repo**: https://github.com/dmno-dev/varlock

Varlock uses `.env.schema` (instead of `.env.example`) to provide a single source of truth for your project's env vars. Schema info is expressed using `@decorator` style comments. Sensitive values can be set in git-ignored `.env.local` files, passed in via the environment, or loaded from secure backends (1Password, AWS Secrets Manager, Vault, and others) with resolver functions.
...
Your `.env.schema` is committed to version control and safe for agents to read and update. The `varlock` CLI loads and validates env vars while masking anything sensitive, and can securely inject env vars into commands.

**NOTE:** If varlock is installed locally via `package.json` (not as a standalone binary), invoke it via your package manager (for example `pnpm exec varlock load`, `bunx varlock load`, `npm exec varlock load`). Check the project's package manager before running CLI commands.
...
- **Safe to read and edit:** `.env.schema` and any other git-committed `.env` files (for example `.env.development`)
...
- Ask the user to edit secret values in their local/gitignored env files or their secret provider (1Password, AWS, or whichever they use). Never fill in secrets yourself.
...
1. Update it in their secret provider and then help them wire it up
...
| `.env.[env]` | Environment-specific tracked config (for example `.env.production`) | Yes |
...
Ensure `.env.schema` and tracked env-specific files are not gitignored (add `!.env.schema`, `!.env.production`, and so on to `.gitignore` if a broad `.env*` rule exists).

### Environment-specific files and precedence

When `@currentEnv` is set in `.env.schema` (for example `@currentEnv=$APP_ENV`), varlock automatically loads matching environment-specific files. Files are applied in increasing precedence order:
...
3. Work through the [schema checklist](#schema-checklist), then run the [validation loop](#validation-loop)

## Schema syntax

### Root decorators (file header)

Root decorators go in comment blocks at the top of the file, before the first item. When a `# ---` divider is present, it marks the end of the header.

| Decorator | Purpose | Default |
|-----------|---------|---------|
| `@currentEnv=$VAR` | Sets which item determines the active environment | none |
...
| `@generateTsTypes(path=./env.d.ts)` | Auto-generate TypeScript env declarations (deprecated alias: `@generateTypes(lang=ts)`) | none |
| `@generatePythonEnv` / `@generateRustEnv` / `@generateGoEnv` / `@generatePhpEnv` / `@generateJavaEnv` / `@generateCsharpEnv` `(path=...)` | Generate a typed env module for that language | none |
| `@import(path, ...keys?)` | Import schema/values from another .env file or directory | none |
| `@plugin(@varlock/name-plugin)` | Load a plugin | none |
| `@setValuesBulk(resolver)` | Inject multiple values from an external source | none |
...
Full list: https://varlock.dev/reference/root-decorators/
...
Full list: https://varlock.dev/reference/item-decorators/

### Data types (`@type=`)

Common types: `string(startsWith=X)`, `string(matches=/regex/)`, `number`, `boolean`, `url`, `port`, `enum(a, b, c)`. Full list with options: https://varlock.dev/reference/data-types/

Plain `string` is the default. Do not add `@type=string`; omit `@type` entirely. Only use `@type` when you need a specific type or string constraints.
...
Full list: https://varlock.dev/reference/functions/

## Setting sensitive values

There are two main approaches, and they can be used together.
...
Each plugin provides its own resolver functions (for example `op()` for 1Password, `awsSecret()` for AWS). See [Plugins](#plugins) below and https://varlock.dev/guides/plugins/ for setup details.
...
- **Interactive prompt:** Set the value to `varlock(prompt)` and run `varlock load`. The user is prompted securely, and the encrypted value replaces the placeholder automatically.
...
- **Pipe via stdin:** To encrypt a value without exposing it in your context (for example a generated key or a value read from another tool), pipe it into `varlock encrypt`:
  ```bash
...
**Single project:** use one `.env.schema` at the repo root.
...
Pick the plugin for the user's provider from https://varlock.dev/plugins/overview/, which lists every supported provider with its setup guide.

## Integrations (frameworks / runtimes)

Pick the official integration for the project's framework; do not guess. Check https://varlock.dev/integrations/overview/ for the specific guide.
...
3. Prefer the integration's recommended entry point (`varlock/auto-load`, Vite plugin, or the guide's equivalent) over ad-hoc `process.env` usage

When a framework integration is active, it handles loading and injecting env vars automatically; `varlock run` is **not needed** for the framework's own dev/build commands. Only use `varlock run -- <cmd>` for other scripts or tools that the integration doesn't cover (for example one-off migrations, CLI tools, non-JS commands).
...
| `varlock codegen` | Explicitly trigger code generation from schema (runs automatically on load by default; `typegen` is a deprecated alias) |

...
`--filter` takes a comma-separated list of selectors. An item is included when it matches at least one non-negated selector (or none are given) and no negated selector:

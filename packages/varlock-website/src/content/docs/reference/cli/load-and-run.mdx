...
One PID 1 duty varlock does not take on is reaping orphaned processes. If your app spawns subprocesses that can outlive their parent, they are reparented to PID 1 and stay as zombies after they exit. In that case run a minimal init above varlock, for example `ENTRYPOINT ["tini", "--", "varlock", "run", "--"]` or `docker run --init`. Signal forwarding works the same either way. See the [Docker guide](/integrations/docker/#pid-1-signals-and-zombie-processes).

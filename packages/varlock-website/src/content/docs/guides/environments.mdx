...
At the very least, you'll often need to inject an environment flag (e.g., `APP_ENV`) and a _secret-zero_ which allows access to the rest of your secrets in deployed environments. For local workflows, consider [device-local encryption](/guides/local-encryption/) to secure local secret-zero handling.

:::caution[Overrides apply at resolution time]
Process overrides are applied when varlock _resolves_ your config: during `varlock run`, `varlock load`, or a framework integration running at dev/build time. Some deployments boot from env values that were already resolved and baked in earlier, for example a framework integration's build output on a serverless platform, or a standalone server image booted without the varlock CLI. In those deployments the process env at boot is not an override source: resolution already happened, and a conflicting env var is surfaced with an error naming the keys rather than being silently ignored. When baking was an implicit fallback (e.g. a Next.js standalone image booted without the varlock CLI) the boot fails; when baking was chosen explicitly (e.g. vite `ssrInjectMode: 'resolved-env'`) the server logs a loud warning and boots on the baked values. To have boot-time env vars respected as overrides, boot through [`varlock run`](/reference/cli/load-and-run/#run) (see the [Docker guide](/integrations/docker/)).
:::

...
These functions generate random values using cryptographically secure randomness (`node:crypto`). In ephemeral environments, it can be helpful to generate values that are unique per run/deployments. For local dev, the [`cache()`](#cache) function can be used to keep a value stable for a period of time.

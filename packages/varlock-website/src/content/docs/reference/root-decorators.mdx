...
A self-contained Python module — a `CoercedEnvSchema` `TypedDict`, a `load_env()` that parses the injected env, and a `SENSITIVE_KEYS` constant. See [Python usage](/integrations/other-languages/).

...
- `--clear-cache`: Clear the active cache store before resolving values, then re-resolve all values (writes new values back unless `--skip-cache` is also set)
...
- `--clear-cache`: Clear the active cache store before resolving values, then re-resolve all values
- `--skip-cache`: Skip cache entirely for this invocation (no reads or writes). This overrides `@cache=disk`/`@cache=memory`.

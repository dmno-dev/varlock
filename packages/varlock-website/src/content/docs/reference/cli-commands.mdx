...
| `filter` | Default [item filter](#filtering-items) for `varlock load` and `varlock run`, using the same selectors as `--filter` (e.g. `"#frontend"` or `"PUBLIC_*,!PUBLIC_DEBUG"`). Only valid together with `loadPath`: it scopes what a shared schema exposes to this package, so a package can point `loadPath` at a shared schema and take just its tagged items with no `.env.schema` of its own. An explicit `--filter` or `_VARLOCK_FILTER` takes precedence, and it is not applied when `--path` overrides `loadPath`. |
...
`--filter` takes a comma-separated list of selectors (the same language is used by `pick`/`omit` on [`@import()`](/reference/root-decorators/#import), by `filter=` on code-generation decorators, and by `varlock.filter` in [`package.json`](#configuration-via-packagejson)):
...
When `--filter` is not passed, the `_VARLOCK_FILTER` env var is used, and failing that `varlock.filter` from `package.json` (which requires `varlock.loadPath`, and is skipped when `--path` is passed).

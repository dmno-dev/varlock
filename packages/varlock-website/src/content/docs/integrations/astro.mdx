...
Marking a value `@dynamic` needs no extra code: server-side reads work as usual. The endpoint + hydration shown below are only needed when **browser code** reads a public+dynamic value (it must be fetched from the backend), which is a niche situation. See [When do you need the client-side machinery?](/guides/dynamic-config/#when-do-you-need-the-client-side-machinery).
...
When the schema contains any public+dynamic keys, this integration automatically injects a route at `/__varlock/public-env` (SSR builds only). If browser code needs one of these values, load it with `loadPublicDynamicEnv()`; this is rarely needed, see the [Astro recipe in the guide](/guides/dynamic-config/#framework-recipes).

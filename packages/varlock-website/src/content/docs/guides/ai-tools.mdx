...
## Global config for agent tools

AI CLI tools are often launched from arbitrary directories and need the same API keys on every machine — without duplicating secrets into each project's `.env.schema`.

Varlock does not auto-load a global schema (that would make two developers' machines behave differently for the same repo). Instead, **opt in** when wrapping agent commands with [`varlock run`](/reference/cli-commands/#run) and an extra [`--path`](/reference/cli-commands/#run) entry point.

Keep a personal schema in your home directory (for example `~/.env.claude`), then pass it with `-p` when you run the tool. Project-local definitions from the current directory still take precedence when the same key exists in both places.

<Tabs syncKey="ai-tool">
  <TabItem label="Claude">
    [Claude Code](https://docs.claude.com/en/docs/claude-code/overview) is Anthropic's CLI tool for AI-assisted coding.

    **Personal schema** — `~/.env.claude`:
  ...
  <TabItem label="Opencode">
    ...
    **Personal schema** — `~/.env.opencode`:
    ...
    Point Opencode auth at env references (see [Opencode setup above](#popular-ai-cli-tool-examples)), then run:
  ...
  <TabItem label="Gemini">
    ...
    **Personal schema** — `~/.env.gemini`:

    ```env-spec title="~/.env.gemini"
    # @sensitive @required
    GOOGLE_CLOUD_PROJECT=exec('op read "op://api-local/google/cloud-project"')
    ...
    **Run from any directory:**
    ...
    ```bash
    varlock run -p ~/.env.gemini -- gemini
    ```

    **Shell alias** (optional):

    ```bash
    alias vgemini='varlock run -p ~/.env.gemini --no-redact-stdout -- gemini'
    ```
  </TabItem>
</Tabs>

### Importing global config into a project

If you prefer project-scoped setup, you can [`@import()`](/guides/import/) a home-directory file from each repo's `.env.schema` instead of passing `-p` every time:
...
That keeps global vars available in the project without requiring a wrapper alias, at the cost of a one-line import per repo. A future `-g` shorthand for `--path ~/.env.global` may simplify the alias pattern further.

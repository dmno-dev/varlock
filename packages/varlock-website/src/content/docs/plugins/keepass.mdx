...
KeePass treats "no master password" and "an empty master password" as different credentials. Omitting `password` means no master password.
...
- `password` (required unless `keyFile` is set): Master password (typically from an env var like `$KP_PASSWORD`). Omit it for a database protected only by a key file
- `keyFile` (optional): Path to a key file, used alone or together with `password`

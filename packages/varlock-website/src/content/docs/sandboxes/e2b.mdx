...
- No human is attached to the broker, so treat its policy as static: allow rules, `block` rules, `@proxy=omit`, and strict egress. To change policy, upload the edited schema and restart the proxy: the pinned token and `--persist-ca` mean running agents just reconnect, with no re-wiring.

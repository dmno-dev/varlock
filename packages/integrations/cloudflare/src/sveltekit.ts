...
import { SVELTEKIT_CLOUDFLARE_SSR_ENTRY_CODE } from './shared-ssr-entry-code';
 ...
 * entry (`@sveltejs/kit/src/runtime/server/index.js`), which is picked up by
 * adapter-cloudflare's generated `_worker.js`. Non-sensitive vars and the
 * `__VARLOCK_ENV` secret should still be uploaded via `varlock-wrangler deploy`.
 *
 * @example
 * ```ts
...
export function varlockSvelteKitCloudflarePlugin(): Array<any> {
  ...
  return [
    ...
    varlockVitePlugin({
      ssrEdgeRuntime: true,
      ssrEntryCode: [SVELTEKIT_CLOUDFLARE_SSR_ENTRY_CODE],
    }),
  ];
}

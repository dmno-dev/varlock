...
- `allowEmpty` (boolean): accepted, but currently has no effect. Empty values skip data type validation entirely, so use [`@required`](/reference/item-decorators/#required)/[`@optional`](/reference/item-decorators/#optional) to control whether an empty value is an error

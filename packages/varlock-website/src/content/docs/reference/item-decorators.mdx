...
- A number value: an error. A numeric secret is already damaged before redaction gets a say, since leading zeros are dropped (an `007123` one-time code resolves to `7123`) and anything past 2^53 is silently rounded. Quote it (`PIN="007123"`) or give it a string type to keep it exact.
...
Composite values (arrays, objects) are measured per element, since each element is registered for redaction on its own. The boolean, number, and `@currentEnv` errors apply only to an explicit per-item `@sensitive`, since `@defaultSensitive=true` sweeps in every item in the file; the length rules apply however the item became sensitive, because redaction registers it either way.

...
When browser code needs a public+dynamic value, it has to be fetched from your backend. Two helpers, both exported from `varlock/env`, cover the round trip.
...
## Boot-time injection (no endpoint)

If your server can rewrite the served HTML when it starts (a common pattern for static SPAs in Docker), you can skip the endpoint and fetch entirely: extract the public+dynamic values with the `--filter` selector language and inject them as a script tag. The runtime picks up `globalThis.__varlockPublicDynamicEnv` automatically at init, so no app code changes are needed, and any existing `loadPublicDynamicEnv()` call short-circuits since values are already hydrated.

Put a placeholder in your HTML entry:

```html title="index.html (in <head>, before your bundle)"
<!--VARLOCK_PUBLIC_DYNAMIC_ENV-->
```

Then replace it at boot:
...
export PUBLIC_ENV_JSON="$(varlock load --filter '@dynamic,!@sensitive' --format json)"

node -e '
  const fs = require("fs");
  const html = fs.readFileSync("dist/index.html", "utf8");
  fs.writeFileSync("dist/index.html", html.replace(
    "<!--VARLOCK_PUBLIC_DYNAMIC_ENV-->",
    `<script>globalThis.__varlockPublicDynamicEnv = ${process.env.PUBLIC_ENV_JSON}</script>`,
  ));
'

...
By default, values are never resolved and plugins are never executed: this is a purely structural transform, safe to run in CI without secrets. (`--vendor-plugins` adds a build-time npm download, but still never resolves values.)
...
- With `--vendor-plugins`, npm `@plugin()` packages are downloaded and copied into `.env-plugins/` and the declarations are rewritten to local paths, so the output resolves with no runtime install. This is the way to run plugins in shell-less, offline, or distroless images. See [plugins in containers](/guides/docker/#distroless-and-shell-less-base-images).
...
- `--vendor-plugins`: Download npm plugins and copy them into the output so no runtime install is needed (needs network at flatten time)

...
**Key/value args:**
- `accountKit` **(required)**: Passbolt account kit (base64-encoded). Reference a config item of type `passboltAccountKit`.
- `passphrase` **(required)**: Passphrase to decrypt your private key. Reference a sensitive config item.

...
To change policy, upload the edited schema the same way, then reload. `proxy reload` validates the schema in its own process before handing it to the running proxy, so the exec needs the project dir and the same bootstrap env the service carries:

```bash
sprite exec -s varlock-broker --dir /home/sprite/proj \
  --env OP_SERVICE_ACCOUNT_TOKEN="$OP_SERVICE_ACCOUNT_TOKEN" -- varlock proxy reload
```

Rule changes apply to agent traffic immediately; a newly added key shows up for newly started `proxy run` commands.

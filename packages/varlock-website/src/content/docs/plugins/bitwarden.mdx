...
The `bw` CLI keeps **one logged-in account per data directory**, so to read from more than one account or server (e.g. personal + work, or bitwarden.com + a self-hosted Vaultwarden) you give each instance its own data dir via `appDataDir`. Set up each dir once with the `bw` CLI (`bw config server …` if needed, then `bw login`), then point an instance at it:

```env-spec title=".env.schema"
# personal account on bitwarden.com
# @initBwp(id=personal, appDataDir="~/.config/Bitwarden CLI Personal")
# work account on a self-hosted Vaultwarden (that dir was configured + logged in separately)
# @initBwp(id=work, appDataDir="~/.config/Bitwarden CLI Work")
# ---

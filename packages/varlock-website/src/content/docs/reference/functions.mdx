...
This returns the full host, not the registrable domain: `domainFromUrl("https://api.example.co.uk")` is `api.example.co.uk`, not `example.co.uk`.

Pass `registrable=true` to narrow the result to the registrable domain (eTLD+1), which is what you usually want for a cookie domain:

```env-spec "registrable"
APP_URL=https://app.example.co.uk
# example.co.uk
COOKIE_DOMAIN=domainFromUrl($APP_URL, registrable=true)
```

This uses the public suffix list, including its private section, so `foo.github.io` stays whole instead of collapsing to `github.io` (a cookie set on a public suffix is dropped by browsers). Hosts that are not on the list at all, like `localhost`, an internal service name, or an IP address, are returned unchanged. A host that is itself a public suffix (`com`, `co.uk`, `github.io`) is a resolution error, since there is nothing valid to return.

...
- `1` / `true`: always trust the blob, skipping the directory check. Use this to hand a blob into an environment with no `.env` files: a sandbox, or a deploy carrying the payload from [`varlock freeze --out -`](/guides/deploy-time-config/#carrying-the-payload-in-an-env-var-instead). auto-load hydrates a Node app from it, and `varlock run` injects its values into any child command. Missing or unusable blobs are hard errors, as is a blob created from a failed resolution.
...
Controls whether [`varlock/auto-load`](/integrations/javascript/) and [`varlock run`](/reference/cli/load-and-run/#run) boot from a [frozen env file](/guides/deploy-time-config/) produced by [`varlock freeze`](/reference/cli/project/#freeze):

- unset (default): use `.varlock-frozen-env` in the working directory if it is present, otherwise resolve normally
- `1` / `true`: require `.varlock-frozen-env`. A missing file is a hard error, so a pipeline that failed to produce it cannot silently fall back to resolving at boot
- `0` / `false`: never use a frozen env file
- any other value: treated as a path to a frozen env file, relative to the working directory unless absolute, and required

A frozen env file that is present but unusable (unreadable, encrypted with no `_VARLOCK_ENV_KEY`, or encrypted with a different key) is always an error, never a fallback to fresh resolution. Note that `1`/`true`/`0`/`false` are the only recognized toggles, so a value like `off` is read as a path and fails as a missing file.

[`varlock load`](/reference/cli/load-and-run/#load) honors the file only when this is set explicitly (`1`/`true` or a path), showing the pinned values with any [`@dynamic=boot`](/reference/item-decorators/#dynamic) keys resolved live. A plain `varlock load` next to a frozen file keeps resolving from the .env files.

This takes precedence over an ambient [`__VARLOCK_ENV`](#__varlock_env) blob, and is independent of [`_VARLOCK_USE_INJECTED_ENV`](#_varlock_use_injected_env): setting that to `0` does not disable frozen env files. `varlock run` flags that change what a resolution produces (`--path`, `--filter`, `--clear-cache`, `--skip-cache`, `--include-internal`) are rejected when a frozen env file is in play.
...
Set on the `varlock` CLI process that `varlock/auto-load` and the framework integrations spawn to resolve your env. If `varlock/auto-load` is preloaded into that CLI process as well (for example via bun's `bunfig.toml` `preload`, where bun also stands in for `node`), it sees this marker and skips resolving instead of spawning another CLI, which would otherwise recurse forever. The CLI removes the marker from its own environment on startup, so processes it spawns (`varlock run` children, `exec()` resolvers, plugins) never inherit it.

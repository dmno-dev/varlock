...
Composite values (arrays, objects) are measured per element, since each element is registered for redaction on its own. The first three errors only apply to an explicit per-item `@sensitive`; `@defaultSensitive=true` sweeps in every item in the file, so items that are sensitive only by default get the warning instead.

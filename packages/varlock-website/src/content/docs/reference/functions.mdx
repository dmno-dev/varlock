...
Maps a value to a new value based on a set of remapping rules. This is useful for translating environment values (such as environment names) to other values, including using regular expressions for flexible matching.
...
- All following arguments are key=value pairs, where the key is the remapped value and the value is what to match against (can be a string or a `regex()` call).
</div>
...
**This cannot be used as a standalone value. It must be used in other functions.**
...
- Takes a single string argument, which is the regex pattern.
</div>
</div>

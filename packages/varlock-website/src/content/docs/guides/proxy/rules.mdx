...
The scheme has two options, `username` and `password`, one per side of the credential pair. Each takes a literal (ordinary config, like a service-account name) or a `$ITEM` reference (a credential the proxy resolves at signing time). On an attached rule **the decorated item fills whichever side you leave unset**, and if you set neither it is the userid, since a single-credential Basic API almost always sends the token as the userid with an empty password (`curl -u "token:"`).
...
# password given, so the item is the userid (GitHub-style TOKEN:x-oauth-basic)
# @proxy(domain="api.github.com", transform={scheme="http-basic", password="x-oauth-basic"})
GH_TOKEN=yourPreferredPlugin()
```

A detached rule names both sides itself, and either or both may be a credential reference:
...
Every referenced item is a consumed credential: managed (the child sees a placeholder), never substituted, and leak-guarded. A rule needs at least one reference, so a detached rule with two literals is a schema error. For the rare API wanting an empty userid with a secret password, set `username=""` explicitly, which pushes the decorated item to the password side.

A literal side can interpolate other config, e.g. `username="acct-${ACCOUNT_ID}"`. Since varlock treats items as sensitive by default, the interpolated item must be marked [`@sensitive=false`](/reference/item-decorators/#sensitive); interpolating a sensitive item is a schema error, because its value would be embedded in the rule rather than resolved at signing time. Credentials always go through `$ITEM` references.

...
- A value that appears inside a non-sensitive item's value: reported on the non-sensitive item. If the sensitive value is 12 characters or more this is an error, since that item carries a real secret into logs, generated types, and client bundles where none of the sensitive item's protections apply. Below that it is a warning about the other direction: redacting the short value will rewrite the containing value everywhere it appears. This one fires even when `allowShortValue` is set, because the collision is confirmed rather than hypothetical.

...
Earlier versions also read a string that starts and ends with `/` as a regex in these positions, quoted or not. That still works and now produces a warning. A future major version will stop interpreting it, and such a string will be compared as-is like any other. Move to `regex()`: `/^abc$/i` becomes `regex("^abc$", "i")`.

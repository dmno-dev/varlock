...
- `--listen <addr>`: For `start`/`run`, bind the proxy off-loopback (e.g. `0.0.0.0`) and serve the built-in WebSocket tunnel on its port so a remote sandbox can reach it via `run --url`. A non-loopback bind mints a per-session data-plane token (pin it with `VARLOCK_PROXY_TOKEN`) that off-loopback clients must present; loopback clients stay exempt and the control endpoint stays loopback-only.
- `--url <wss>` + `--token <token>`: For `run`, target a proxy on another machine (a broker started with `--listen`) over the tunnel, instead of a local session. The token is the broker's data-plane token (also readable from `VARLOCK_PROXY_TOKEN`). The local-proxy flags (`--sandbox`, `--port`, `--cert-dir`, `--listen`) don't apply with `--url`.
...
varlock proxy start --listen 0.0.0.0

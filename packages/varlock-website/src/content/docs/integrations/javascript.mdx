...
If any check fails, auto-load falls back to the CLI. Control it explicitly with [`_VARLOCK_USE_INJECTED_ENV`](/reference/reserved-variables/#_varlock_use_injected_env): `0` always re-resolves, `1` always trusts the blob, skipping the directory and drift checks. A blob that resolved with errors is refused in every mode: trust is about where the blob came from, not whether it worked. `varlock run` applies the same rules when it finds a blob, which covers non-Node workloads. Trust mode is how you hand an env into an environment with no `.env` files at all, like a remote sandbox (see the [E2B](/sandboxes/e2b/#passing-resolved-values) and [Fly.io](/sandboxes/flyio/#passing-resolved-values) guides) or a deploy carrying a [frozen payload](/guides/deploy-time-config/#carrying-the-payload-in-an-env-var-instead) in one variable.


## Boot via `varlock run`

A less invasive way to use varlock with your application is to run your application via [`varlock run`](/reference/cli/load-and-run/#run).

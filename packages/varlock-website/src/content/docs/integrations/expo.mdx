...
Both stacks use the same [`@varlock/expo-integration`](https://npmx.dev/package/@varlock/expo-integration) package. Only the Babel preset and Metro base config differ between Expo and React Native CLI projects.
...
<Tabs syncKey="expo-rn">
  ...
  <TabItem label="React Native CLI">
    :::caution[Requirements]
    - Node.js v22 or higher
    - React Native v0.72 or higher (ships `@react-native/metro-config`)
    :::
  </TabItem>
</Tabs>
...
1. **Install varlock and the integration package**
...
1. **Wrap your Metro config**
    ...
    Wrap your Metro config with `withVarlockMetroConfig`. This automatically configures Metro to resolve varlock's subpath exports (such as `varlock/env`) and initializes the `ENV` proxy in the main Metro process.
    ...
    - Initializes the varlock `ENV` proxy in the main Metro process. Metro forks worker processes for Babel transforms; the wrapper ensures the environment is initialized in the main process (needed for Expo Router `+api` server routes and dev-time console redaction).
    :::
...
- Non-string values (for example number, boolean) are properly typed and coerced
...
If you find you are not getting type completion on `ENV`, you may need to add your generated type files (`env.d.ts` by default) to your `tsconfig.json`'s `include` array.
:::
...
Usually this env var will be defaulted to something like `development` in your `.env.schema` file, and you can override it when running commands.

<Tabs syncKey="expo-rn">
  <TabItem label="Expo">
    For Expo projects, set it in your `package.json` scripts or via [EAS environment variables](https://docs.expo.dev/eas/environment-variables/):
  ...
  <TabItem label="React Native CLI">
    For React Native CLI projects, set it in your `package.json` scripts:
    ...
    ```diff lang="json" title="package.json" add=/APP_ENV=[a-z]+/
    {
      "scripts": {
        ...
        "android:staging": "APP_ENV=staging react-native run-android",
        "ios:staging": "APP_ENV=staging react-native run-ios",
      }
    }
    ```
  </TabItem>
...
The babel plugin inlines **static** values into your app code at build time. By default that means every non-sensitive value; mark a public value [`@dynamic`](/reference/item-decorators/#dynamic) to opt it out of inlining.
For the shared model and tradeoffs, see the [Static vs Dynamic Config guide](/guides/dynamic-config/).

Values that are not inlined (sensitive values, and anything `@dynamic`) are only directly accessible in **server routes** (`+api` files). Referencing one from native code logs a build-time warning, and reading it at runtime in native will fail. If native code needs a runtime-fresh public value, expose it from a server route and hydrate over the network; see the [Expo recipe in the guide](/guides/dynamic-config/#loading-from-a-server-endpoint).

...
Interactive unlock needs a TTY. In CI (or any non-interactive context) provide credentials explicitly instead:
...
- **Cannot unlock without an interactive terminal** — you're in a non-TTY context (e.g. CI). Provide `sessionToken=$BWP_SESSION` or `masterPassword=...` to `@initBwp()`.

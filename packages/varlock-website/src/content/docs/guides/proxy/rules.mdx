...
| `secretIn` | Which side of the `user:password` pair the secret occupies: `password` _(default)_ or `username`, for APIs that take a single token as the userid with an empty password (`curl -u "token:"` style). `username` must be unset in that case. |

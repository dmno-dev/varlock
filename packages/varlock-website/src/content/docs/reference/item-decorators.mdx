...
- The [`@currentEnv`](/reference/root-decorators/#currentenv) item: an error. The current env name drives conditional imports and `forEnv()`, and is echoed by most tools in the stack.
...
Composite values (arrays, objects) are measured per element, since each element is registered for redaction on its own. An element that isn't a string is never redacted at all, so a sensitive `array(number)` or `record(number)` follows the same rule as a bare number: an error when `@sensitive` is explicit, a warning when it came from `@defaultSensitive`. These apply however the item became sensitive, because redaction registers it either way: an item swept in by `@defaultSensitive=true` needs `@sensitive=false` (or `@public`) just like an explicitly marked one. The `@currentEnv` rule is the exception, and only fires on an explicit per-item `@sensitive`. Builtin `VARLOCK_*` variables are exempt.

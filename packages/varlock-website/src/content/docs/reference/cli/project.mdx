...
Scans your project files for sensitive config values that must not appear in plaintext. Loads your varlock config, resolves all `@sensitive` values, then checks files for any occurrences of those values.
...
If varlock is installed as a project dependency, the hook command will be automatically prefixed with your package manager (for example `npx varlock scan`).
...
**Positional arguments:**
- `<name@version>`: The plugin to install, with an exact version (for example `my-plugin@1.2.3`).

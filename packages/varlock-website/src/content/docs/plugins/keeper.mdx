...
1. **Create a Secrets Manager Application** in the Keeper Admin Console
    ...
    Navigate to **Secrets Manager** → **Applications** → Click **Create Application**. Give it a descriptive name (for example "varlock-prod").
...
API_KEY=keeper("XXXX/custom_field/MySecretLabel")

# By record title instead of UID
SMTP_PASSWORD=keeper("SMTP Credentials/field/password")
```
...
**Key/value args:**
- `token` (required): Base64-encoded Secrets Manager config token. Reference a config item of type `keeperSmToken`.

...
These functions generate random values using cryptographically secure randomness (`node:crypto`). They are typically used with [`cache()`](#cache) to generate a value once and reuse it according to your active cache mode.
...
Wraps any resolver to cache its result.

- In `disk` cache mode, values persist across invocations and are encrypted at rest.
- In `memory` cache mode, values are reused only within the current varlock process.
- In `disabled` mode, `cache()` is effectively bypassed.

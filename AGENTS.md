...
Docs content lives in `packages/varlock-website/src/content/docs/` (`.mdx`). The same rules apply to `skills/varlock/SKILL.md` and the skills under `packages/varlock-website/public/.well-known/agent-skills/`. When writing or editing docs prose, keep the tone plain and direct, like an engineer wrote it:

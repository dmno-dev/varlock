...
// pinned via VARLOCK_PROXY_TOKEN so we already know it. Schema keys resolve
// from the process env, so envs carries the bootstrap: usually just your
// plugin's secret-zero (shown: a 1Password service account).

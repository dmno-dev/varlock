...
| Option | Meaning |
|---|---|
| `password` | The item holding the password (this scheme's consumed secret; replaces the generic `secretKey` name). Defaults to the decorated item on an attached rule. Takes an item **name**, never a literal password; if an API genuinely needs a fixed password, put it in an item. |
| `username` | The username: a literal, or `$ITEM_NAME` to use another item's value. Omitted = empty username. |
...
Note the asymmetry: `username` can be a `$` reference because a username is wire-visible config and the reference resolves to its value. Options that name credential items (`password` here, `secretKey`/`keyId` elsewhere) take the plain item **name** instead, never a `$` reference; the proxy resolves their real values at signing time, so secrets stay out of rule data. The schema validator rejects `$` references there.

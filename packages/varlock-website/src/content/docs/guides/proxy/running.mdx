...
**Signals and exit codes** follow `varlock run`: `SIGTERM`, `SIGINT`, `SIGHUP`, and `SIGQUIT` are forwarded to the child so it can run its own shutdown handlers, varlock waits for it, and the child's exit status is propagated (`128+N` when it dies from a signal). The proxy session is closed after the child exits. See [Signals and exit codes](/reference/cli/load-and-run/#run) for the full contract, including the opt-in `SIGKILL` escalation.

...
Unlike the built-in patterns, these see string contents as written, so a key that isn't a bare identifier (`config.get('app.database.url')`) still matches. Code inside comments is skipped for both.

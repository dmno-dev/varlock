...
- `--allow-reload`: For `start`/`run`, enable live policy hot-reload via `proxy refresh` (off by default — the reload channel is unauthenticated on a shared uid; only enable behind a sandbox).

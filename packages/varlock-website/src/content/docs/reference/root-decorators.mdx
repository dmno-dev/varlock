...
Code-generation decorators turn your schema into generated code. Most commonly this is **type definitions** for your language, but [plugins](/guides/plugins/) can register generators that emit other kinds of code. See the [Code generation guide](/guides/code-generation/) for the full picture.

Varlock ships per-language type generators — add one decorator per output file you want:

- [`@generateTsTypes()`](#generatetstypes) — TypeScript
- `@generatePythonTypes()` — Python (`TypedDict`)
- `@generateRustTypes()` — Rust
- `@generateGoTypes()` — Go
- `@generatePhpTypes()` — PHP (phpstan)
...
`@generateTypes(lang=ts, path=...)` still works as a TypeScript-only alias for `@generateTsTypes`, but is deprecated. For other languages, use the matching `@generate*Types` decorator (`@generateTypes(lang=py, ...)` will error with a pointer to `@generatePythonTypes`).
:::

...
- A bare name matches **any** directory with that name, at any depth. `fixtures` skips both `./fixtures` and `./src/deep/fixtures`.
- An entry containing a separator is **anchored at the scan root**. `apps/docs` skips only that directory, leaving a `docs` elsewhere in the tree alone.
- A leading `/` or `./` anchors a single name to the scan root, so `./fixtures` skips the top-level one but not `src/deep/fixtures`.

Trailing separators are ignored, and `\` is accepted as a separator so Windows-style entries work.

...
Most apps do not need this: on platforms that rebuild on every env change, static public values already update with each change. See [Do you actually need dynamic+public?](/guides/dynamic-config/#do-you-actually-need-dynamicpublic). On Vercel in particular, env changes trigger a redeploy (and rebuild), so dynamic+public mainly matters for self-hosted deployments or promoted build artifacts.

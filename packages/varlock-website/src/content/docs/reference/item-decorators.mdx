...
You can also use the [`forEnv()` helper](/reference/functions/#forenv) to dynamically set the value of `@required` based on the current environment.
</div>
...
You can also use the [`forEnv()` helper](/reference/functions/#forenv) to dynamically set the value of `@optional` based on the current environment.
</div>

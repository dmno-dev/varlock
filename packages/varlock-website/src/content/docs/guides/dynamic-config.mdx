...
Prerendered routes run `dehydrate()` at build time, so they get a build-time snapshot. For those, serve the values from a server route instead and call `loadPublicDynamicEnv({ endpoint })` in the browser, as in the other recipes.

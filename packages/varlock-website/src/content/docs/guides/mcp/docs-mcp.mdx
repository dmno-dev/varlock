...
Clients that auto-discover MCP servers can read the server card at [`/.well-known/mcp/server-card.json`](https://varlock.dev/.well-known/mcp/server-card.json) (also served at `/.well-known/mcp.json`). No authentication is required.

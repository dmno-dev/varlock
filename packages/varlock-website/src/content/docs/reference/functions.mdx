...
This uses the public suffix list, including its private section, so `foo.github.io` stays whole instead of collapsing to `github.io` (a cookie set on a public suffix is dropped by browsers). Hosts with no registrable domain, like `localhost` and IP addresses, are returned unchanged. A host that is itself a public suffix (`co.uk`) is a resolution error, since there is nothing valid to return.

...
- `allowedDomains` (string[]): List of allowed hosts, matched in full and case-insensitively. An entry without a port allows any port, so `[localhost]` accepts `http://localhost:3000`; add a port to pin it (`["localhost:3000"]`). Entries are hosts only: a scheme or path in an entry is an error. A bare string is treated as a single host; list two or more as an array

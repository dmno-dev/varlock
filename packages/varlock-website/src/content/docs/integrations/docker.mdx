...
The baked snapshot is authoritative: it was resolved at build time, so values passed in the container's runtime environment (e.g. `docker run -e REDIS_URL=...`) cannot be validated or applied to it. If a runtime env var conflicts with the snapshot for a schema-declared key, varlock fails loudly (at boot or on the first request, depending on when the bundled runtime loads) with an error naming the keys, instead of silently ignoring the runtime values. The fix is to boot through `varlock run` (see the Runtime injection tab), which re-resolves against the runtime environment with full validation and coercion. To accept the baked values anyway, set `_VARLOCK_ALLOW_ENV_SNAPSHOT_CONFLICTS=1`. Blob-only deployments, where no schema values are set in the runtime env, are unaffected.

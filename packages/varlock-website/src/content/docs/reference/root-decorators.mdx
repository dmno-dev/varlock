...
- `exposeEnv`: where the coerced `ENV` object lives (defaults to `global`).
  ...
  - `local` — export a package-local typed `ENV` from the generated file (with **no** global augmentation). Use this in monorepos where multiple packages have different schemas. Requires a `.ts` (or `.js`) output path, and you import from the generated file (e.g. `import { ENV } from './env'`).
...
# @generateTsTypes(path=./env.ts, exposeEnv=local, processEnv=none, importMetaEnv=none)
```
</div>
...
A self-contained Rust module — a serde-derived `Env` struct, a cached `ENV` (`LazyLock`) plus a `load()` function, and a `SENSITIVE_KEYS` constant. Requires the `serde` and `serde_json` crates. See [Rust usage](/integrations/other-languages/).
...
A self-contained Go package — an `Env` struct, a cached `Get()` (`sync.OnceValue`) plus a `Load()` function, and a `SensitiveKeys` map. See [Go usage](/integrations/other-languages/).
...
A self-contained PHP class — a readonly `Env` with a cached `Env::instance()` plus a static `load()`, and a `SENSITIVE_KEYS` constant. Requires PHP 8.1+. See [PHP usage](/integrations/other-languages/).

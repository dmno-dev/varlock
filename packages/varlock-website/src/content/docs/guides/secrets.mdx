...
Marking an item `@sensitive` is a claim that varlock will keep the value out of your output. Redaction makes good on that by replacing the value wherever it appears, which only works for a string long enough not to occur in ordinary text. Some values can never be protected that way, and varlock handles them differently depending on whether you asked for it.

| The value | Inherited from `@defaultSensitive` | Explicit `@sensitive` |
| --- | --- | --- |
| a number or boolean | not sensitive | error |
| an array or object of numbers or booleans | not sensitive | error |
| the [`@currentEnv`](/reference/root-decorators/#currentenv) item | not sensitive | error |
| under 3 characters | error | error |
| under 12 characters | warning | warning |
| a secret embedded in a non-sensitive item's value | error, on the non-sensitive item | error, on the non-sensitive item |

**Numbers, booleans and the env flag are never sensitive by default.** Redaction only replaces strings, so a number is never masked and a boolean holds no secret anyway; the current env name is a mode that every tool in your stack echoes. `@defaultSensitive=true` was never a claim about these items, so a `PORT=3000` or `DEBUG=true` in a hand-written schema is simply treated as public, and `varlock explain` will say so. Writing `@sensitive` on one yourself is a claim, and a wrong one, so that is an error. For a numeric secret, make it a string with `@type=string` or by quoting the value (`PIN="007123"`), which also preserves the leading zeros and the precision past 2^53 that a number drops.

This decision is made from the type your schema declares or implies, never from a value in an environment-specific file. An untyped `API_KEY=` whose `.env.local` placeholder happens to be `12345` stays sensitive and is rejected there, rather than quietly becoming public in one environment. Note that a demoted item also becomes [`@static`](/reference/item-decorators/#static) by default, so a boolean that must be read at runtime needs an explicit `@dynamic`.

**Composite values** are checked per element, since each element is registered for redaction on its own. A sensitive array or object must be made of strings, and a long array with a one-character element is still a one-character match.

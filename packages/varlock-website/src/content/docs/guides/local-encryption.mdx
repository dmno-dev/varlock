...
**What we do to prevent this.** Windows binaries are Authenticode-signed via Azure Artifact Signing, macOS binaries are Developer ID signed and notarized (both the `VarlockEnclave.app` helper and the standalone `varlock` CLI, see [macOS code signing](/getting-started/installation/#macos-code-signing)), and no binary is compressed with an executable packer (packers such as UPX are a well-known trigger for these detections).
...
1. **Navigate to your Doppler project config**
    ...
    Go to the [Doppler dashboard](https://dashboard.doppler.com), select your project, and open the config (for example `dev`, `stg`, `prd`) you want to access.
...
- `config` (required): Config name (for example `dev`, `stg`, `prd`, or a branch config)
- `serviceToken` (required): Doppler service token. Reference a config item of type `dopplerServiceToken`.

...
- `allowShortValue` (`boolean`, default `false`): when `true`, silences the [short-value warning](/guides/secrets/#values-varlock-cant-protect) for secrets that are legitimately short and can't be lengthened (a one-time code, a PIN). Redaction is unchanged; this only records that the collision risk was read and accepted. It does not apply to values under 3 characters: those are an error when you wrote `@sensitive` yourself and a warning when `@defaultSensitive` swept them in, and the option suppresses neither.
...
`@dynamic=boot` marks a stricter subset: the value is bound at process start on each instance, so it cannot be fixed at deploy time either. [`varlock freeze`](/guides/deploy-time-config/#keys-supplied-at-boot) leaves such items out of the pin, and they are resolved and validated against the schema at boot. Use it for a platform-assigned `PORT`, pod or machine identity, or anything an operator sets with `docker run -e`. It must be written literally, and it is per item (there is no root default). Because a boot value does not exist before boot, nothing bound earlier may depend on it: a static or deploy-pinned item whose value or decorators reference a boot item is a schema error on every load, and the fix is to mark that item `@dynamic=boot` too. Root decorators cannot reference a boot item either (`@redactLogs=$X`, `@cache`, `@disable`, `@import(enabled=...)`, plugin init), since they are evaluated before boot, and the [`@currentEnv`](/reference/root-decorators/#currentenv) item cannot be boot, since it decides which env files load.

| Value | Bound at | Meaning |
| --- | --- | --- |
| `false` (or `@static`) | build | Eligible for build-time replacement by integrations |
| `true` | deploy | Never inlined at build. Read at runtime from the server's env, which under `varlock freeze` is the frozen file |
| `boot` | boot | Resolved on each instance at process start, even under `varlock freeze` |

Items can also be selected by this state via the `@dynamic` selector (or `!@dynamic` for static items) in the [`--filter` language](/reference/cli-commands/#filtering-items); boot items are included by `@dynamic`. See the [Static vs Dynamic Config guide](/guides/dynamic-config/) for the full model.
...
Attaches one or more tags to the item. Tags don't affect resolution or validation; they're used to select items with the [`--filter` CLI flag](/reference/cli-commands/#filtering-items) on `varlock load`/`varlock run`, with `pick`/`omit` on [`@import()`](/reference/root-decorators/#import) (e.g. `@import(../../, pick=[#billing])`), and with the [`filter=` arg](/reference/root-decorators/#code-generation) on `@generate*` code-generation decorators (e.g. `--filter="#billing"`, `@generateTsTypes(filter=#billing)`). _Can be called multiple times; tags from every call accumulate (duplicates collapse)._
...
**Function form** `@proxy(domain=..., [path], [method], [block], [approval], [keys], [substituteIn], [rules], [transform])`:

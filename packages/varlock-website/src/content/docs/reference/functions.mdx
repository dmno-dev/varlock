...
There is currently no way to match such a string literally in a `remap()` match slot. This is tracked in [#1089](https://github.com/dmno-dev/varlock/issues/1089), which will let a leading `\/` escape mark the value as literal.

A path only runs into this when it both starts and ends with a slash, so most paths are unaffected. As a top-level config value (e.g., `MY_PATH=/usr/local/bin`), slash-containing strings are always plain strings.
:::

...
- **The signing secret is consumed, not substituted.** The child never holds it: it only ever sees a placeholder, and the proxy applies the real value itself. `substituteIn` and `maxOccurrences` therefore don't apply, and if the placeholder shows up anywhere in any request the request is blocked, since there is no legitimate reason for the child to send it. The `keyId` item is wire-visible (it is an API key id, not a secret key), so it substitutes normally like a `keys=` entry.

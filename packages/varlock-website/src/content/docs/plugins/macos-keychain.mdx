...
`fix-access` preserves existing Keychain item ACLs. If macOS still prompts after fixing access, you can explicitly recreate a generic-password item so VarlockEnclave owns it:

```sh
varlock keychain take-ownership --account "my-app:myenv:API_KEY"
```

This can reset per-app Keychain ACLs on that item, so use it only when you want that ownership rewrite.

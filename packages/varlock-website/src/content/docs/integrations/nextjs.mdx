...
:::caution[Root override applies to every Next.js app]
Apps without a `.env.schema` may fail at dev or build time (for example, `process.env.__VARLOCK_ENV is not set`). See the [Monorepos guide](/guides/monorepos/) for schema layout, incremental adoption, Turborepo, CI, and Docker patterns.
:::

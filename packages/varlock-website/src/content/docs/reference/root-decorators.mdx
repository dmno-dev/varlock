...
- `auto`: Controls whether generation runs automatically on every load (defaults to `true`). Set to `false` to disable automatic generation and instead run [`varlock codegen`](/reference/cli-commands/#codegen) explicitly.
...
If you prefer to generate manually rather than on every load, set `auto=false` and run [`varlock codegen`](/reference/cli-commands/#codegen) explicitly:

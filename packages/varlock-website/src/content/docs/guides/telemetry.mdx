...
- Whether the env graph was loaded (`graph_loaded`) and a coarse error category when the run failed (`error_code`: one of `parse_error`, `plugin_error`, `schema_error`, `validation_error`, `resolution_error`, `load_error`): no error messages or file paths

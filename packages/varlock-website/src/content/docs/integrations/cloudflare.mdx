...
Where it does matter is when build and deploy are decoupled: for example, building client assets once and deploying that same build to several environments, where deploy-time resolution gives each environment different values. For that setup, see the [Cloudflare recipe in the guide](/guides/dynamic-config/#framework-recipes); the endpoint serves whatever the worker was deployed with, not per-request-fresh values.

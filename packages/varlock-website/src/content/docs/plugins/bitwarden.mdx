...
- **Zero-config authentication** - Provide your machine account access token
...
1. **Create a machine account** in your Bitwarden organization
    ...
    Provide a name (for example "Production App") and save it.
...
4. **Provide a name** (for example "Production App")
...
- `sessionTtl` (default `forever`): how long the unlocked CLI session token is cached before varlock forces a fresh `bw unlock`. `forever` keeps the token until the CLI session is invalidated externally (`bw lock`/`bw logout`, an external `bw unlock`, or account policy), at which point the next load re-unlocks automatically; set for example `"1h"` to force periodic master-password re-auth.
...
The `bw` CLI keeps **one logged-in account per data directory**, so to read from more than one account or server (for example personal + work, or bitwarden.com + a self-hosted Vaultwarden) you give each instance its own data dir via `appDataDir`. Set up each dir once with the `bw` CLI (`bw config server …` if needed, then `bw login`), then point an instance at it. Since a data-dir path is specific to your machine, reference it from a per-developer value rather than hardcoding it:
...
**Key/value args:**
- `accessToken` (required): Machine account access token. Reference a config item of type `bitwardenAccessToken`.
...
- `cacheTtl` (optional): cache resolved values for the specified duration (for example `"5m"`, `"1h"`, `"1d"`, or `forever` to cache until manually cleared). For cache mode behavior and CLI cache controls, see the [Caching guide](/guides/caching/).
...
- `sessionToken`: a pre-obtained CLI session token (for example `$BWP_SESSION`). When provided and non-empty, it's used as-is instead of auto-unlocking.
- `masterPassword`: master password used to unlock the vault non-interactively (for example in CI). Reference a `@sensitive` config item.
- `sessionTtl`: how long the auto-unlocked session token is cached before a fresh `bw unlock` (default `forever`, kept until the CLI session is invalidated externally; for example `"1h"` to force periodic master-password re-auth).
...
- UUIDs must contain 32 hexadecimal characters and 4 hyphens

...
:::note[`regex()` function]
`regex("pattern")` takes the pattern as a quoted string, with optional flags as a second argument. Prefer the `/pattern/` form when it fits, since it is more concise. Reach for `regex()` when the pattern contains a `,`, a space, or a `)`, or when it has enough `/` in it that escaping each one hurts readability.

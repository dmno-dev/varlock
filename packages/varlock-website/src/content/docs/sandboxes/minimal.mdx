...
The session networking these recipes use (the `.local.min.internal` hostnames, the proxy on port 7654, the host alias IP, and the `--network` / `--ingress` flags) is not in Minimal's docs yet and is not a stable contract: names, ports, and flags may change. We verified every recipe on this page end to end on macOS with `min` 0.5.4. If a command here does not exist in your install, you need a newer build; ask the Minimal team. The same surface on Linux is untested.
:::
...
- **A proxy routes by hostname.** The Minimal daemon runs an HTTP proxy on `127.0.0.1:7654` on your Mac. From inside any session it is reachable at `100.64.255.254:7654` (the host alias below); default-network sessions also see it on their own `127.0.0.1:7654`, [own-ip](#own-ip-sessions-and-future-enforcement) sessions do not, so the alias form is the one to script against. The port in the URL picks the port inside the target session, nothing to declare up front. WebSockets pass through (both Upgrade and CONNECT tunneling), which matters here because varlock's [tunnel](/guides/proxy/running/#remote-proxy-start---expose--proxy-run---url) is a WebSocket.
...
Declare varlock (it ships in Minimal's package registry) and the tunnel token in `minimal.toml`, activate, then wrap the agent command with `proxy run --url` pointed at the host alias. varlock self-wires the agent's placeholder env and CA certs over the tunnel:

```bash
cd your-project
min init                       # scaffolds minimal.toml
min add --session varlock      # plus whatever your agent needs (node, curl, ...)
```
...
Three setup facts worth knowing up front:

- Session packages come from `minimal.toml` at activation. Inside a running session, the in-sandbox `min` helper can install a tool on the spot (`min add --session <pkg>`), but that install is scoped to the shell that ran it; anything you need durably goes in the file, and host-side `min add --session` only edits the file for the next activation.
...
- The registry's `varlock` package can lag npm by a release. If you need the newest varlock, `npm i -g --prefix ~/.local varlock` (with `node` in your packages) puts it in `~/.local/bin`, which is on the session `PATH`; a bare `npm i -g` fails because the global prefix points into the read-only package store.
...
For repeatable runs, declare the wrapped agent as a task and run it in an ephemeral session with `min task run`. The task carries its own packages and inherits the token from the shell that runs it (gated by the same user policy as above):

```toml title="minimal.toml"
[tasks.agent]
packages = ["varlock", "curl"]
env_vars.VARLOCK_PROXY_TOKEN = { inherit = true }
exec = "bash ./agent-task.sh"   # the wrapper lives in a script: task strings are Nickel-interpolated, so a literal %{...} breaks them
```

```bash
min task run agent   # uploads the project, runs the task, streams output, tears the session down
```

We verified the full broker path this way. Two related commands: `min session exec <session> -- <cmd>` runs an ad-hoc command in an existing session with its packages on `PATH` (also verified with the wrapper; output is complete and quoting survives as of 0.5.4, though a command that backgrounds a process holds the stream open until that process exits), and `min session list --json` / `min session destroy --force` cover the rest of the lifecycle. `min session run <session> <task>` exists too, but as of 0.5.4 it composes neither the session's `[session.vars]` nor task-level inherits, so it cannot carry the token yet. `attach` gives a persistent shell that survives detach and re-attach.
:::
...
```bash
# hub: activate a session for your broker project (varlock in its packages) with a stable name
...
HTTP_PROXY=http://100.64.255.254:7654 HTTPS_PROXY=http://100.64.255.254:7654 \
  varlock proxy run --url ws://varlock.local.min.internal:8080 -- your-agent-command
```

varlock's tunnel client [honors `HTTP_PROXY` / `HTTPS_PROXY`](/guides/proxy/running/#remote-proxy-start---expose--proxy-run---url) and dials through Minimal's proxy, and the WebSocket upgrade passes through, so no shim is needed. Once connected, varlock wires the child's own proxy env over the tunnel, so the agent's requests ride the varlock tunnel rather than Minimal's proxy directly. The alias address works from default and [own-ip](#own-ip-sessions-and-future-enforcement) workers alike (both verified), which is why the recipe uses it rather than the session-local `127.0.0.1:7654`.
...
- With default sessions the hostname route is a convention, not a boundary: sessions share a network namespace, so a worker could also reach the hub's port directly. Own-ip workers are separate network identities and reach the hub only through the proxy, which is the shape Minimal's egress policy will eventually enforce.
...
What we saw on the installed release (0.5.4, macOS): own-ip activation and `--ingress` publishing both work, and both broker paths pass from an own-ip session: the host broker via the alias, and a hub session by hostname through the alias proxy (`100.64.255.254:7654`). The session-local `127.0.0.1:7654` does not exist in an own-ip namespace, so scripts that hardcode it fail there; the recipes above use the alias for that reason.
...
- **The 7654 proxy trusts whoever can reach it** (single-user model), and every session can reach it, so any session can route to any peer session's ports. Do not forward 7654 off the machine; if something a session serves needs a public URL, point a tunnel (cloudflared, ngrok, ...) at that one routed port, not at the proxy.
...
**Port 7654 is fixed, and the daemon claims it only at startup.** If something else on your Mac holds 7654 when the daemon starts, it warns (`session hostnames will not route: ... bind: address already in use`) and carries on without routing; it does not retry once the port frees. Free the port, `min stop`, and the next `min` command restarts the daemon. If routing looks dead with no warning in sight: `lsof -nP -iTCP:7654 -sTCP:LISTEN`. (A request for an unknown hostname returns an empty `502`, so a `502` means the proxy is up and the session name in the URL is wrong.)

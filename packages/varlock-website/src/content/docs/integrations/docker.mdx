...
The baked snapshot is authoritative: it was resolved at build time, so values passed in the container's runtime environment (e.g. `docker run -e REDIS_URL=...`) cannot be validated or applied to it. If a runtime env var conflicts with the snapshot for a schema-declared key, varlock surfaces it with a message naming the keys instead of silently ignoring the runtime values. When baking was an implicit fallback (e.g. a Next.js standalone image booted without the varlock CLI), the boot fails (at boot or on the first request, depending on when the bundled runtime loads); set `_VARLOCK_ALLOW_ENV_SNAPSHOT_CONFLICTS=1` to accept the baked values anyway. When baking was chosen explicitly (e.g. vite `ssrInjectMode: 'resolved-env'`), varlock logs a loud warning and boots on the baked values: that artifact carries its own config by design, so it must be rebuilt (or built with a non-baking inject mode) for runtime env to apply. Otherwise, to have runtime env respected, boot through `varlock run` (see the Runtime injection tab), which re-resolves against the runtime environment with full validation and coercion. Blob-only deployments, where no schema values are set in the runtime env, are unaffected.

...
- A boolean value: an error. Redacting it rewrites every `true`/`false` in logs and proxied response bodies, and a boolean is never a secret worth that cost.

...
Plugins have access to cache helpers that can be used to avoid repeated network round trips, or to hold short-lived tokens. Plugins that support caching expose params (like `cacheTtl`) to let the user opt into it. This still respects global cache mode and CLI flags. Each plugin can decide how to cache things and it will often depend on the shape of the external API.
...
Caching is mostly useful for local development, where there may be many invocations of varlock and strong [local encryption](/guides/local-encryption/) (for example biometric-protected keys) is available. The default mode (`auto`) picks the most secure persistent option available:
...
[`varlock load`](/reference/cli/load-and-run/#load), [`varlock run`](/reference/cli/load-and-run/#run), and [`varlock printenv`](/reference/cli/load-and-run/#printenv) support:
...
Use [`varlock cache`](/reference/cli/cache-and-codegen/#cache) to inspect and clear the **disk cache**:

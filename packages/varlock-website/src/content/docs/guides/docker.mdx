...
If you already use [`varlock flatten`](/reference/cli-commands/#flatten) for a monorepo build, add `--vendor-plugins`. It downloads each `@plugin()` package and copies it into the output (`.env-plugins/`), rewriting the declarations to local paths. The flattened directory then resolves with **no runtime npm fetch, no shell, and no trust prompt**, because the download and confirmation happened during the build:

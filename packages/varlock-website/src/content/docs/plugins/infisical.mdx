...
```
:::caution[Note for EU users] Infisical default `siteUrl` is `https://app.infisical.com`.
If you are using the Infisical EU Cloud, you need to pass to `@initFisical` also the `siteUrl` param with the value `https://eu.infisical.com`
[Infisical Universal Auth Documentation](https://infisical.com/docs/documentation/platform/identities/universal-auth)
:::

    ...
    for (const pluginDecorator of installPluginDecorators) {
      ...
      try {
        ...
        // we will assume its a npm module name - `packageName` / `packageName@version`
          ...
          const semverRange = validRange(versionDescriptor);
          ...
          while (currentDir) {
            ...
            if (await pathExists(candidatePluginPath)) {
              ...
              if (versionDescriptor && !satisfies(packageVersion, versionDescriptor)) {
          ...
          if (!pluginSrcPath) {
            ...
            } else if (!valid(versionDescriptor)) {

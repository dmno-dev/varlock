...
The region and service need no configuration: they are parsed from the inbound request's credential scope, so one rule covers every AWS service and region the client talks to. See the [plugin docs](/plugins/aws-sigv4/) for all options and limitations.

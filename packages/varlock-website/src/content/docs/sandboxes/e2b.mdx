...
- No human is attached to the broker, so treat its policy as static: allow rules, `block` rules, `@proxy=omit`, and strict egress. To change policy, write the edited schema with `files.write`, run `varlock proxy stop` in the broker, and rerun the same proxy start command (and readiness wait) from the setup above: the pinned token and `--persist-ca` mean running agents just reconnect, with no re-wiring.

...
- The cache shares its encryption key with your `varlock("local:...")` env values, so one approval covers both. Reading a cached value can be what raises the [unlock panel](/guides/local-encryption/#the-unlock-panel), and when it does the panel lists the cache alongside the env files under that key, with how many cached values it holds and which plugins and files filled it.

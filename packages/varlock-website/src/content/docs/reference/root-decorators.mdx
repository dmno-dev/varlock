...
Use the `regex('...')` form (or a quoted slash-delimited literal) rather than a bare `/.../ ` literal: bare decorator args cannot contain spaces, commas or parentheses, so realistic patterns would not survive parsing. Anything that is neither stays a schema error instead of silently scanning nothing.

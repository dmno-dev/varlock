...
Expansion equivalent: `ref(DB_HOST)` === `${DB_HOST}` (and also `$DB_HOST`)
...
If varlock has a [plugin](/plugins/overview/) for your provider (1Password, AWS, Vault, …), use its resolver (for example `op()`) instead of shelling out. Plugins handle auth, caching, and validation. Reach for `exec()` only for tools without a plugin, or for custom logic.
:::
...
An unquoted match value that looks like a valid regex (that is, `/something/` with optional flags) will be treated as a regex pattern. If you need to match a literal string containing slashes (like a file path), wrap it in quotes.
```env-spec
...
ITEM=remap($VAR, "/usr/lib/", matched)
```
Note that as a top-level config value (for example `MY_PATH=/usr/local/bin`), slash-containing strings are always treated as plain strings.
:::
...
Generates a time-based one-time password (TOTP) code from a shared secret, the same codes an authenticator app shows. Useful for CLIs that require a 2FA code on every invocation, for example `aws sts get-session-token --token-code 123456`.
...
- `key=S` option: use an explicit cache key instead of the auto-generated one. Useful when the same cached value needs to be shared across files or when you want a stable key that doesn't change with resolver edits.
...
- Store encrypted payloads (`varlock("local:...")`) in local override files (such as `.env.local`)
...
- `keychain` (optional): Name of a specific keychain to search (for example `"System"`)

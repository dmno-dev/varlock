...
The same override can be set with the `_VARLOCK_REDACT_STDOUT` environment variable (`true`/`1` or `false`/`0`) when you can't easily change the command — e.g. in a wrapper script or CI config. The CLI flag takes precedence over the env var.

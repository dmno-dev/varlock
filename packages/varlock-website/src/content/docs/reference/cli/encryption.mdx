...
- `--ignore` / `-i`: Directory name to exclude from code scanning (can be specified multiple times). Matched by name at any depth, and merged with [`@auditIgnorePaths()`](/reference/root-decorators/#auditignorepaths) from your schema.

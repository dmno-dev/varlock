...
- **What would be unlocked**: the key ids, with a count of how many values each covers.
- **For how long**: the scope.
- Optionally the project name and path, shown dimmed underneath. That part is sent by the CLI, so it is treated as decoration: it changes the wording, never the decision.

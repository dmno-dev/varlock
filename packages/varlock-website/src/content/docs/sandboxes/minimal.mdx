...
- **microVM** isolation gives the guest its own loopback, so it cannot reach the host's `127.0.0.1`. The broker binds loopback by default; start it with `--expose` to bind off-loopback and serve the [tunnel](/guides/proxy/running/#remote-proxy-start---expose--proxy-run---url), then point the guest at that address. If `minimal run` still cannot connect through the proxy on your platform, the guest network path is the gap to look at, not a misconfigured schema.

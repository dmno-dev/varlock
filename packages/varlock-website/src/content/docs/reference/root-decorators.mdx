...
<div>
### `@cache`
**Value type:** `boolean | "auto" | "memory" | "disk" | "disabled"`

Sets global cache mode for [`cache()`](/reference/functions/#cache) and plugin cache APIs.

**Modes:**
- `auto` (default): choose mode automatically
...
In `auto` mode, Varlock uses:
- `memory` in CI, or when local encryption is using file-based fallback
- `disk` otherwise
...
```env-spec
# Disable all caching for this schema
# @cache=false
# ---
INSTANCE_ID=cache(randomUuid())
```

...
- **`varlock typegen` command** - Added environment-independent type generation as a first-class command. (Since renamed to [`varlock codegen`](/reference/cli/cache-and-codegen/#codegen), with `typegen` kept as a deprecated alias.)

...
| `transform` | Request signing config, e.g. `transform={scheme="hmac-sha256", stringToSign="{timestamp}{method}{path}{body}", signatureHeader="X-Signature"}`. The proxy signs the final outbound request with the item's real value, which is consumed by the signer and never sent. Built-in schemes: `hmac-sha256`, `hmac-sha512`, and `http-basic` (which composes the `Authorization: Basic` header the proxy sends, since base64 hides a placeholder from substitution); plugins add more, e.g. `aws-sigv4`. The consumed secret defaults to the decorated item on an attached rule and is required on a detached one, written as a reference (`secretKey=$PARTNER_SECRET`, or `password=$REGISTRY_PASSWORD` for `http-basic`). See [Request signing](/guides/proxy/rules/#request-signing) for per-scheme options. |

...
Reference any `VARLOCK_*` variable in your schema just like any other variable:

```env-spec title=".env.schema"
# @currentEnv=$VARLOCK_ENV
# ---
MY_BRANCH=$VARLOCK_BRANCH
...
DB_URL=postgres://prod-db.example.com/myapp
```

```env-spec title=".env.staging"
DB_URL=postgres://staging-db.example.com/myapp
```

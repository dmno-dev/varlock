...
To keep each package's `ENV` isolated, use module mode. It emits a package-local file that re-exports the runtime `ENV` typed to *that* package's schema, with no global augmentation:

```env-spec title="packages/api/.env.schema"
# @generateTsTypes(path=./env.ts, env=module)
```
...
Runtime behavior is identical (it's the same underlying proxy); only the types are scoped locally. The output path must be a `.ts`/`.js` file (not `.d.ts`) since it contains a runtime re-export.

...
A self-contained Python module — a `CoercedEnvSchema` `TypedDict`, a `load_env()` that parses the injected env, an eager typed `ENV`, and a `SENSITIVE_KEYS` constant. See [Python usage](/integrations/other-languages/).
...
A self-contained Rust module — a serde-derived `Env` struct, a `load()` function, and a `SENSITIVE_KEYS` constant. Requires the `serde` and `serde_json` crates. See [Rust usage](/integrations/other-languages/).
...
A self-contained Go package — an `Env` struct, a `Load()` function, and a `SensitiveKeys` map. See [Go usage](/integrations/other-languages/).
...
A self-contained PHP class — a readonly `Env` with a static `load()` and a `SENSITIVE_KEYS` constant. Requires PHP 8.1+. See [PHP usage](/integrations/other-languages/).

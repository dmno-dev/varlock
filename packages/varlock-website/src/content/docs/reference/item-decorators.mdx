...
- A value that appears inside a non-sensitive item's value: an error on the non-sensitive item. It carries the secret into logs, generated types, and client bundles, where none of the sensitive item's protections apply. Mark that item `@sensitive` too, or build it from a reference so the secret part stays separate.

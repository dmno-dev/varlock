...
If no `@defaultRequired` applies to an item, it is required. This includes items defined in a file that does not set `@defaultRequired` itself, such as a `.env.local` or an [imported](/guides/import/) file, even when the root schema sets one. Mark those items with `@optional`, or set `@defaultRequired` in that file.

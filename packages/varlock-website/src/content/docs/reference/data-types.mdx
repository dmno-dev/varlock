...
- `matches` (string|RegExp): Regular expression pattern to match. Use `/pattern/flags` syntax, quoting the whole literal if the pattern contains a comma or space, or `regex("pattern", "flags")` (see [regex-like strings](/reference/functions#regex-like-strings))
...
- `matches` (string|RegExp): Regular expression pattern the full URL must match. Use `/pattern/flags` syntax, quoting the whole literal if the pattern contains a comma or space, or `regex("pattern", "flags")` (see [regex-like strings](/reference/functions#regex-like-strings))

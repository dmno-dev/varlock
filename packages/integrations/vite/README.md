...
--
By default, Vite will load `.env` files automatically, and does support loading multiple files (`.env.local`, `.env.developent`, etc)
and uses dotenv-expand to support variable expansion.

However using varlock gives you additional features like validation, type generation, and more.

Here is a comparison of the two:

| Feature | Varlock | Vite (dotenv) | Notes |
| --- | --- | --- | --- |
| Loading env-specific files | Yes | Yes | Vite uses its `mode` flag to set the environment (`.env.[mode]`) |
| Variable expansion | Yes | Yes |
| Using env vars in `vite.config` file | Yes | No |
| Type generation | Yes | No |
| Validation | Yes | No |
| Log redaction | Yes | No |
| Control over client exposed vars | Yes | Yes | Vite uses the `VITE_` prefix, while varlock is more flexible

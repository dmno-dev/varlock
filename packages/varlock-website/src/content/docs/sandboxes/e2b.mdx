...
Import [`varlock/auto-load`](/integrations/javascript/) at the top of the orchestrator: varlock resolves your values (from plugins, `.env.local`, and your other env files), validates them against your schema, and redacts them in the orchestrator's logs.
...
// --allow-reload lets you apply schema edits later without a restart; the
// reload channel is only reachable from inside the broker, not by agents.
...
await broker.commands.run(
  `cd /home/user/proj && ${VARLOCK} proxy start --expose --port 8080 --cert-dir /home/user/proj/.varlock-ca --persist-ca --allow-reload > /home/user/proxy.log 2>&1`,
  {
    background: true,
    timeoutMs: 0,
    envs: {
...
- No human is attached to the broker, so its policy must run unattended: allow rules, `block` rules, `@proxy=omit`, and strict egress. To change policy, write the edited schema with `files.write` and run `${VARLOCK} proxy reload` via `commands.run`: the proxy validates the edit in its own context before applying, and a broken edit is refused and reported back. Rule changes apply to agent traffic immediately; a newly added key shows up for newly started `proxy run` commands.
...
- **Anywhere in a request.** Substitution happens anywhere in a request, not only one static header per host, and policy can match on path and method.

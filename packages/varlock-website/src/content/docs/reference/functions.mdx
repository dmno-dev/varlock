...
A string is treated as a regex when it starts and ends with `/`, with optional flags (`d`, `i`, `g`, `m`, `s`, `u`, `v`, `y`) after the closing `/`.

Detection happens after parsing, on the value itself, so quoting does not change the result. That makes quotes useful as pure transport: a `,`, a space, or a `)` ends an unquoted value, so wrap the whole literal in quotes when the pattern contains one. A `{n,m}` quantifier is the common case:

```env-spec
# the comma in {7,40} would end an unquoted value, so quote the literal
# @type=string(matches="/^[0-9a-f]{7,40}$/")
APP_COMMIT_SHA=

# flags stay inside the quotes with it
# @type=string(matches="/^[0-9A-F]{7,40}$/i")
APP_COMMIT_SHA_ANY_CASE=
```
...
Since `/something/` looks like a valid regex, a value like `/usr/lib/` is treated as a regex pattern in contexts that support them (like `remap()` match values). Quoting does not opt out of this, because detection runs on the value after its quotes are removed:

```env-spec
# both of these are read as a regex matching "usr/lib"
ITEM=remap($VAR, /usr/lib/, matched)
ITEM=remap($VAR, "/usr/lib/", matched)
```

To match such a value exactly, write it as a regex with the inner slashes escaped and the pattern anchored:

```env-spec
SRC=/usr/lib/
# matches only the exact value
ITEM=remap($SRC, "/^\/usr\/lib\/$/", matched)
```

Escaping without the wrapping slashes (`"\/usr\/lib\/"`) does not do this. It opts out of regex detection, but the value is then compared as the literal string `\/usr\/lib\/`, backslashes and all, so it never matches.

A path only runs into any of this when it both starts and ends with a slash, so most paths are unaffected. As a top-level config value (e.g., `MY_PATH=/usr/local/bin`), slash-containing strings are always plain strings.
:::

:::note[`regex()` function]
`regex("pattern")` takes the pattern source, with optional flags as a second argument. It is written **without** the surrounding slashes -- `regex("/^abc$/")` would match the slashes themselves, so it is rejected with an error.

Prefer a `/pattern/` literal when it fits, quoted if it contains a `,`, a space, or a `)`. Reach for `regex()` when the pattern has enough `/` in it that escaping each one hurts readability.

```env-spec
# @type=string(matches=regex("^[0-9a-f]{7,40}$"))
APP_COMMIT_SHA=

# flags go in the second argument
...
Extracts the domain (host) from a URL, dropping the protocol, credentials, port, path, query, and fragment. Useful for deriving things like a cookie domain or an allowed origin from a URL you already have.

The value is parsed as a URL, so the result is the host a request would actually be sent to. A value with no scheme (`example.com`, `example.com:8080`) is treated as a bare host. The result is lowercased, and internationalized domains are converted to punycode.

This returns the full host, not the registrable domain: `domainFromUrl("https://api.example.co.uk")` is `api.example.co.uk`, not `example.co.uk`. To rewrite specific hosts, wrap it in [`remap()`](#remap). If you need a parent domain that covers every subdomain, such as a cookie or email domain, set the root domain as its own item and build the URLs from it with [`concat()`](#concat) instead of taking them apart.

The item's type is inferred as `domain` unless you set one explicitly.

An empty or undefined input resolves to undefined. Anything else that cannot be parsed as a URL, or a URL with no host at all (like `mailto:someone@example.com`), is a resolution error.

```env-spec "domainFromUrl"
API_URL=https://api.example.com/v1
# api.example.com
API_DOMAIN=domainFromUrl($API_URL)

# use it to build another value
ALLOWED_ORIGIN=concat("https://", domainFromUrl($API_URL))
```
</div>
...
Use the [`varlock cache`](/reference/cli/cache-and-codegen/#cache) CLI command to view or clear the **disk cache**.
...
- Use [`varlock reveal`](/reference/cli/encryption/#reveal) when you need to inspect a decrypted value interactively

...
A marker set when the runtime booted from a build-time baked env snapshot rather than a fresh resolution, so child/worker processes inheriting the [`__VARLOCK_ENV`](#__varlock_env) blob apply the same semantics (conflicting runtime env values fail the boot, see [`_VARLOCK_ALLOW_BAKED_ENV_CONFLICTS`](#_varlock_allow_baked_env_conflicts)).

...
- For **local dev**, a temporary key is minted when the Vite plugin is imported, so dev runtimes spawned by other plugins (for example Nitro's worker) inherit it; Cloudflare dev targets fall back to plaintext (see [Local development](/guides/encrypted-deployments/#local-development))

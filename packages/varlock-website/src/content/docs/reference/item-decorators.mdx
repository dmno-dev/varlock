...
- A number value: an error. Make it a string instead, with `@type=string` or by quoting the value (`PIN="007123"`). A numeric secret is already damaged before redaction gets a say: leading zeros are dropped (an `007123` one-time code resolves to `7123`) and anything past 2^53 is silently rounded.
...
Only booleans are demoted rather than rejected. For every other type the value could hold a real secret, and quietly making it public would also make it `@static` and eligible to be inlined into a build, so varlock stops instead of guessing.

...
For **container** deployments (Docker, Docker Compose, Kubernetes), see the [Docker guide](/guides/docker/). It covers CI-only validation vs runtime injection, multi-stage builds, monorepo import workarounds, and when not to ship the CLI in your production image.

...
On Workers, all values reach the worker at runtime (hydrated from bindings at boot), so [`@dynamic`](/reference/item-decorators/#dynamic) mainly affects the **client build**: with the Vite plugin approach, public+dynamic keys are excluded from build-time inlining in browser assets. Worker-side code keeps using `ENV.KEY` as usual.
For the shared model and tradeoffs, see the [Static vs Dynamic Config guide](/guides/dynamic-config/).

There is no automatically injected endpoint on Cloudflare. For browser code that needs a public+dynamic value at runtime, add a route in your worker that serves `getPublicDynamicEnv()`:
...
Note that worker values update on deploy (or when you update the underlying secret/var), not per request. The endpoint serves whatever the worker booted with.

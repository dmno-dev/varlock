...
Flexible duration type. Accepts human-readable strings (`"1h"`, `"30m"`, `"500ms"`, `"2days"`) or bare numbers (interpreted as milliseconds), and outputs a number in the unit you specify.
...
Same parser is used by `cache(..., ttl=...)` and the plugin `cacheTtl` option, so any string that works there also works here.
</div>
</div>
